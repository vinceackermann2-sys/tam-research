...
APP_NAME = "tam-research-aera-v26-5-issue505-end-to-end-systems"
VOLUME_NAME = "tam-research-data"
RESULT_PATH = "/vol/aera-v26/issue505-end-to-end-systems/result.json"
MAX_GPU_SECONDS = 600
RESEARCH_ISSUE = 505
SOURCE_MAIN = "4dece4ef767d8de58b74acd091e1adb55009c5ab"
...
def preflight() -> dict:
    ...
    if result_path.exists():
        raise RuntimeError(
            f"refusing duplicate issue505 end-to-end systems run because result exists: {RESULT_PATH}"
        )
    ...
    if blobs != expected:
        raise RuntimeError(f"issue505 frozen blob drift: got={blobs} expected={expected}")

    protocol = repaired.repair1_protocol()
    for key, expected_value in _required_protocol().items():
        if protocol.get(key) != expected_value:
            raise RuntimeError(
                f"issue505 repaired systems protocol drift: {key}={protocol.get(key)!r} expected={expected_value!r}"
            )

    if historical.CHECKPOINT_RELATIVE_DIR != "/vol/aera-real-language/v25-dev-seed8471":
        raise RuntimeError("issue505 checkpoint path drift")
    hashes = historical.checkpoint_hashes(historical.CHECKPOINT_RELATIVE_DIR)
    if set(hashes) != {"aera.pt", "transformer.pt"}:
        raise RuntimeError(f"issue505 checkpoint hash inventory drift: {sorted(hashes)}")

    contract = repaired.cpu_contract_preflight_repair1()
    if contract["gpu_authorized_by_issue503"] is not False:
        raise RuntimeError("issue505 inherited repair contract unexpectedly authorizes GPU")
    ...
    return {
        ...
        "systems_gate_authorized_by_issue505": True,
...
def run_gate() -> dict:
    ...
    print(
        "AERA_V26_5_ISSUE505_END_TO_END_SYSTEMS_L4_START_JSON="
        + json.dumps(
            {
    ...
    print(
        "AERA_V26_5_ISSUE505_END_TO_END_SYSTEMS_SUMMARY_JSON="
...
def main() -> None:
    ...
    print(
        "AERA_V26_5_ISSUE505_END_TO_END_SYSTEMS_PREFLIGHT_JSON="
