...
APP_NAME = "tam-research-aera-v26-6-issue546-end-to-end-systems"
VOLUME_NAME = "tam-research-data"
RESULT_PATH = "/vol/aera-v26/issue546-end-to-end-systems/result.json"
MAX_GPU_SECONDS = 600
RESEARCH_ISSUE = 546
...
ISSUE539_TRIGGERS = (542, 543, 544)
ISSUE539_RUNS = (33684268881, 33684274250, 33684290786)
ISSUE539_JOBS = (100427851578, None, 100428014282)
...
PREFLIGHT_MARKER = "AERA_V26_6_ISSUE546_END_TO_END_SYSTEMS_PREFLIGHT_JSON="
L4_START_MARKER = "AERA_V26_6_ISSUE546_END_TO_END_SYSTEMS_L4_START_JSON="
RESULT_MARKER = "AERA_V26_6_ISSUE546_END_TO_END_SYSTEMS_RESULT_JSON="
SUMMARY_MARKER = "AERA_V26_6_ISSUE546_END_TO_END_SYSTEMS_SUMMARY_JSON="
...
def preflight() -> dict:
    ...
    if result_path.exists():
        raise RuntimeError(
            f"refusing duplicate issue546 end-to-end systems run because result exists: {RESULT_PATH}"
        )
    ...
    if blobs != expected:
        raise RuntimeError(f"issue546 frozen blob drift: got={blobs} expected={expected}")
    ...
    for key, expected_value in required.items():
        if protocol.get(key) != expected_value:
            raise RuntimeError(
                f"issue546 evaluator protocol drift: {key}={protocol.get(key)!r} expected={expected_value!r}"
            )

    if base.CHECKPOINT_RELATIVE_DIR != "/vol/aera-real-language/v25-dev-seed8471":
        raise RuntimeError("issue546 checkpoint path drift")
    hashes = base.checkpoint_hashes(base.CHECKPOINT_RELATIVE_DIR)
    if set(hashes) != CHECKPOINT_HASH_KEYS:
        raise RuntimeError(
            f"issue546 checkpoint hash inventory drift: got={sorted(hashes)} expected={sorted(CHECKPOINT_HASH_KEYS)}"
        )
    if not all(_valid_sha256(value) for value in hashes.values()):
        raise RuntimeError("issue546 checkpoint hash value drift")

    contract = evaluator.cpu_contract_preflight_issue530()
    if contract["gpu_authorized_by_cpu_preflight"] is not False:
        raise RuntimeError("issue546 inherited #530 contract unexpectedly authorizes GPU")
    ...
    return {
        ...
        "issue539_triggers": list(ISSUE539_TRIGGERS),
