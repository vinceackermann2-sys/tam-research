...
APP_NAME = "tam-research-aera-v6-integrated-l4"
...
def preflight() -> dict:
    ...
    model = HardwareAwareAERATextLMV6(cfg).eval()
    ...
    if y.shape != (2, 31, cfg.vocab_size) or not torch.isfinite(y).all():
        raise RuntimeError("AERA v6 mixed-precision preflight failed")
...
def run_gate(repo_full_name: str = "", issue_number: int = 0) -> dict:
    ...
    torch.manual_seed(7360)
    torch.cuda.manual_seed_all(7360)
    ...
    base = HardwareAwareAERATextLMV6(cfg).to(device=device, dtype=dtype).eval()

    _comment(repo_full_name, issue_number,
        "🧪 **Integrated AERA-v6 L4 gate started** — BMM hard experts, BF16-safe adaptive depth, Flash chunk attention, predictive recurrent state, true hard top1/top2 routing, fast-memory read path. Inference only; timeout=600s; no retry.")
    ...
    _comment(repo_full_name, issue_number,
        "📊 **Integrated AERA-v6 L4 results**\n\n"
        f"- batch1 lean vs dense: **{b1['lean_vs_dense']:.2f}x** ({b1['lean']['ms']:.2f} vs {b1['dense']['ms']:.2f} ms)\n"
        f"- batch8 lean vs dense: **{b8['lean_vs_dense']:.2f}x** ({b8['lean']['ms']:.2f} vs {b8['dense']['ms']:.2f} ms)\n"
