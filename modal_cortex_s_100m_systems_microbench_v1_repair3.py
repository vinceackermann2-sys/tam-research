...
APP_NAME = "cortex-s-v0-100m-systems-microbench-v1-repair3"
...
def verify_data_zero_gpu(source_sha: str) -> dict:
    """Hash corpus and verify repair2 layout plus repair3 Python scoping on CPU."""

    from architectures.cortex_s.experiments.scale100m_2b.systems_microbench_v1_repair3 import (
        ENGINEERING_SEED as REMOTE_ENGINEERING_SEED,
        validate_microbench_protocol,
        zero_gpu_repair3_contract_probe,
    )

    if REMOTE_ENGINEERING_SEED != ENGINEERING_SEED:
        raise RuntimeError("repair3 launcher/remote engineering seed drift")
    ...
    if protocol.get("full_training_authorized") is not False or protocol.get("next_stage_authorized") is not False:
        raise RuntimeError("repair3 protocol unexpectedly authorizes a later stage")
    ...
    if data_path.exists() or dispatch_path.exists() or result_path.exists():
        raise RuntimeError("systems microbenchmark repair3 namespace is already consumed")

    data = _verify_frozen_data()
    contract = zero_gpu_repair3_contract_probe()
    if contract.get("status") != "PASS":
        raise RuntimeError("repair3 zero-GPU contract did not pass")
    result = {
        "status": "PASS",
        "scientific_status": "ZERO_GPU_SYSTEMS_DATA_LAYOUT_AND_SCOPING_GATE_ONLY",
        "repair_namespace": "microbench-v1-repair3",
        "source_sha": source_sha,
        "engineering_seed": ENGINEERING_SEED,
        "data": data,
        "repair3_contract": contract,
    ...
    volume.commit()
    print(json.dumps({"repair3_zero_gpu": result}, indent=2), flush=True)
    return result
...
def h100_microbenchmark(source_sha: str) -> dict:
    """Exactly one single-use repair3 H100 systems measurement; never 2B training."""
    ...
    if not data_path.exists():
        raise RuntimeError("repair3 zero-GPU data/layout/scoping gate is missing")
    ...
    if data_gate.get("status") != "PASS" or data_gate.get("source_sha") != source_sha:
        raise RuntimeError("repair3 zero-GPU gate does not authorize this exact source SHA")
    if dispatch_path.exists() or result_path.exists():
        raise RuntimeError("systems H100 microbenchmark repair3 namespace already consumed")
    ...
    volume.commit()
    print(json.dumps({"repair3_h100_started": dispatch}, indent=2), flush=True)
    ...
    try:
        ...
        volume.commit()
        print(json.dumps({"repair3_failure": failure}, indent=2), flush=True)
        raise

    result = dict(result)
    result["repair_namespace"] = "microbench-v1-repair3"
    result["full_training_authorized"] = False
    result["next_stage_authorized"] = False
    result_path.write_text(json.dumps(result, indent=2), encoding="utf-8")
    volume.commit()
    print(json.dumps({"repair3_result": result}, indent=2), flush=True)
    return result


@app.local_entrypoint()
def main(phase: str = "microbench-v1-repair3", source_sha: str = ""):
    if phase.strip().lower() != "microbench-v1-repair3":
        raise ValueError("only phase='microbench-v1-repair3' is supported")
