...
APP_NAME = "cortex-s-v0-100m-2b-preflight-v9-fused-linear-ce-compile-boundary"
...
def _validate_payload(source_sha: str, source_tree: str, harness_sha: str) -> tuple[str, str, str]:
    ...
    if ENGINEERING_SEED in forbidden:
        raise RuntimeError("v9 engineering seed is retired, consumed, or scientifically reserved")
...
def _dependency_smoke() -> dict[str, Any]:
    ...
    if packaging_version != PACKAGING_VERSION:
        raise RuntimeError(f"packaging version drift: {packaging_version}")
    if liger_version != LIGER_KERNEL_VERSION:
        raise RuntimeError(f"liger-kernel version drift: {liger_version}")
    from packaging.version import Version
    from liger_kernel.transformers import LigerFusedLinearCrossEntropyLoss

    if Version(packaging_version) != Version(PACKAGING_VERSION):
        raise RuntimeError("packaging Version import/version mismatch")
    if LigerFusedLinearCrossEntropyLoss is None:
        raise RuntimeError("Liger FLCE import returned no class")
    return {
        "packaging_version": packaging_version,
        "packaging_version_import_ok": True,
        "liger_kernel_version": liger_version,
...
def _verify_frozen_inputs() -> dict[str, Any]:
    ...
    for key, value in required_status.items():
        if status.get(key) != value:
            raise RuntimeError(f"fused candidate contract drift for {key}: {status.get(key)!r}")
    ...
    if "runner = _normalize_fused_training_runner(model, runner)" not in step_source:
        raise RuntimeError("fused fallback normalization missing")
    boundary_source = inspect.getsource(candidate.FusedLinearCETrainingRunner._liger_loss_eager)
    if "@torch.compiler.disable" not in boundary_source:
        raise RuntimeError("Liger Dynamo boundary missing")
    if "self._liger_loss(weight, flat_hidden, flat_targets)" not in boundary_source:
        raise RuntimeError("Liger Dynamo boundary no longer encloses FLCE")
    feature_source = inspect.getsource(candidate.forward_training_features)
    if "torch.compiler.disable" in feature_source:
        raise RuntimeError("CORTEX feature trunk was excluded from compilation")
    compile_source = inspect.getsource(candidate._compile_fused_linear_ce_runner)
    if "torch.compile(runner, mode=training_module.COMPILE_MODE, fullgraph=False)" not in compile_source:
        raise RuntimeError("compiled runner contract drift")

    hashes["metadata"] = meta
    hashes["fused_candidate_status"] = status
    return hashes
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if root.exists():
        raise RuntimeError("reserved v9 result namespace already exists; fail closed")
    ...
    payload = {
        "status": "PASS",
        "classification": "ZERO_GPU_FUSED_LINEAR_CE_COMPILE_BOUNDARY_PREFLIGHT_V9",
        "phase": PHASE,
...
def reserve_h100_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    smoke = zero.get("dependency_smoke", {})
    if not smoke.get("packaging_version_import_ok") or not smoke.get("liger_fused_linear_ce_import_ok"):
        raise RuntimeError("dependency import smoke did not pass before dispatch")
    if marker_path.exists() or result_path.exists():
        raise RuntimeError("v9 engineering attempt is already consumed")
    marker = {
        "status": "H100_DISPATCH_CONSUMED",
        "classification": "ENGINEERING_SINGLE_ATTEMPT_MARKER_V9_FUSED_LINEAR_CE_COMPILE_BOUNDARY",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "source_sha": source,
        "source_tree": tree,
...
def _prove_fused_loss_cuda(model: Any) -> dict[str, Any]:
    ...
    from architectures.cortex_s.fused_linear_ce_v1 import FusedLinearCETrainingRunner, integration_status
    ...
    if runner.model is not model or not runner._use_liger or runner._liger_loss is None:
        raise RuntimeError("CUDA fused-loss runner did not select pinned Liger on the same model")
    if model.lm_head.weight is not model.token_emb.weight:
        raise RuntimeError("tied lm-head/token-embedding identity drift")
    boundary_source = _inspect.getsource(runner._liger_loss_eager)
    if "@torch.compiler.disable" not in boundary_source:
        raise RuntimeError("CUDA runner is missing the explicit Liger compile boundary")
    status = integration_status()
    return {
        "liger_selected_on_cuda": True,
        "liger_kernel_version": importlib.metadata.version("liger-kernel"),
        "compile_boundary": status.get("compile_boundary"),
        "cortex_feature_trunk_compiled": status.get("cortex_feature_trunk_compiled"),
        "liger_loss_executes_eager": status.get("liger_loss_executes_eager"),
        "fallback_runner_normalized": status.get("fallback_runner_normalized"),
...
def h100_preflight(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if result_path.exists():
        raise RuntimeError("v9 RESULT.json already exists; refusing rerun")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("v9 systems preflight requires CUDA")
    device = torch.device("cuda")
    if torch.cuda.get_device_capability(device)[0] < 9:
        raise RuntimeError("v9 systems preflight requires H100/SM90-class CUDA")
    ...
    try:
            ...
            with memory_lean_grouped_triton_scan_training_builder():
                with fused_linear_ce_training_builder():
                    raw = training_module.run_h100_calibration(
                        data_dir=DATA_DIR,
                        output_path="/tmp/cortex_s_v9_fused_linear_ce_compile_boundary_calibration.json",
                        source_sha=source,
                    )
        finally:
        ...
        if raw.get("execution") != "compiled":
            raise RuntimeError(f"v9 candidate did not remain compiled: {raw.get('compile_error')}")
        ...
        fused_pass = bool(fused_cuda_proof.get("liger_selected_on_cuda")) and fused_cuda_proof.get("compile_boundary") == COMPILE_BOUNDARY
        status = "SYSTEMS_PREFLIGHT_PASS" if calibration_pass and dispatcher_pass and fused_pass else "SYSTEMS_PREFLIGHT_FAIL"
        result = {
            "status": status,
            "classification": "ENGINEERING_SYSTEMS_PREFLIGHT_V9_FUSED_LINEAR_CE_COMPILE_BOUNDARY_ONLY",
            "phase": PHASE,
            "source_sha": source,
            "source_tree": tree,
            "harness_sha": harness,
        ...
        failure = {
            ...
            "classification": "ENGINEERING_SYSTEMS_PREFLIGHT_V9_FUSED_LINEAR_CE_COMPILE_BOUNDARY_ERROR",
            "phase": PHASE,
            "source_sha": source,
            "source_tree": tree,
            "harness_sha": harness,
