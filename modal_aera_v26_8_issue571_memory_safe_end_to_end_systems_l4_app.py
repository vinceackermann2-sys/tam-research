...
APP_NAME = "aera-v26-8-issue571-memory-safe-e2e-systems-l4"
...
MEMORY_SAFE_ADAPTER_BLOB = "1eeaa80adb2ae960e2a8dad06031c4ed5ca99203"
MEMORY_SAFE_CPU_TEST_BLOB = "a6b3fc81239b4e7b4c6d2185f9d21465384876a0"
...
ISSUE570_HEAD = "06cb135a6ec92f59ffbd1a2a215da8367092c079"
ISSUE570_CPU_RUN = 33740176638
ISSUE570_CPU_JOB = 100599997754
ISSUE570_MERGE = SOURCE_MAIN

ISSUE566_TRIGGER = 566
ISSUE566_BOUND_MAIN = "6e9471ca86fed0438bd25dd314040a16e637f2be"
...
def _summary(result: dict) -> dict:
    ...
    return {
        ...
        "issue571_gate_metadata": result.get("issue571_gate_metadata", {}),
    }
...
def preflight() -> dict:
    ...
    if result_path.exists():
        raise RuntimeError(
            f"refusing duplicate issue571 memory-safe systems run because result exists: {RESULT_PATH}"
        )

    blobs = {
        "memory_safe_adapter": _git_blob_sha(Path(memory_safe.__file__)),
    ...
    expected = {
        "memory_safe_adapter": MEMORY_SAFE_ADAPTER_BLOB,
    ...
    if blobs != expected:
        raise RuntimeError(f"issue571 frozen blob drift: got={blobs} expected={expected}")

    contract = memory_safe.cpu_contract_preflight_issue569()
    if contract["gpu_authorized_by_cpu_preflight"] is not False:
        raise RuntimeError("issue571 inherited CPU contract unexpectedly authorizes GPU")
    if contract["systems_measurement_performed"] is not False:
        raise RuntimeError("issue571 CPU preflight unexpectedly performed systems measurement")
    if contract["checkpoint_loaded"] is not False:
        raise RuntimeError("issue571 CPU preflight unexpectedly loaded checkpoint")
    ...
    for key, expected_value in required.items():
        if protocol.get(key) != expected_value:
            raise RuntimeError(
                f"issue571 memory-safe protocol drift: {key}={protocol.get(key)!r} "
                f"expected={expected_value!r}"
            )

    if base.CHECKPOINT_RELATIVE_DIR != "/vol/aera-real-language/v25-dev-seed8471":
        raise RuntimeError("issue571 checkpoint path drift")
    hashes = base.checkpoint_hashes(base.CHECKPOINT_RELATIVE_DIR)
    if set(hashes) != CHECKPOINT_HASH_KEYS:
        raise RuntimeError(
            f"issue571 checkpoint hash inventory drift: got={sorted(hashes)} "
            f"expected={sorted(CHECKPOINT_HASH_KEYS)}"
        )
    if hashes != CHECKPOINT_HASHES:
        raise RuntimeError(f"issue571 checkpoint hash drift: got={hashes} expected={CHECKPOINT_HASHES}")
    if not all(_valid_sha256(value) for value in hashes.values()):
        raise RuntimeError("issue571 checkpoint hash value drift")
    ...
    return {
        ...
        "issue570_head": ISSUE570_HEAD,
        "issue570_cpu_run": ISSUE570_CPU_RUN,
        "issue570_cpu_job": ISSUE570_CPU_JOB,
        "issue570_merge": ISSUE570_MERGE,
        "issue566_consumed_pre_result": True,
        "issue567_inert_pre_gpu": True,
        "issue558_result_sha256": ISSUE558_RESULT_SHA256,
...
def run_gate() -> dict:
    import torch
    from tam_research.aera_v26_8_issue569_end_to_end_systems_memory_safe import (
        run_end_to_end_systems_v26_8_memory_safe,
    )
        ...
        + json.dumps(
            {
                ...
                "source_main": SOURCE_MAIN,
                "memory_safe_adapter_blob": MEMORY_SAFE_ADAPTER_BLOB,
    ...
    result = run_end_to_end_systems_v26_8_memory_safe()
    result["issue571_gate_metadata"] = {
        "source_main": SOURCE_MAIN,
        "source_tree": SOURCE_TREE,
        "memory_safe_adapter_blob": MEMORY_SAFE_ADAPTER_BLOB,
        "memory_safe_cpu_test_blob": MEMORY_SAFE_CPU_TEST_BLOB,
