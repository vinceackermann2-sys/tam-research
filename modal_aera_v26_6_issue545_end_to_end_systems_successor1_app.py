...
APP_NAME = "tam-research-aera-v26-6-issue545-end-to-end-systems-successor1"
VOLUME_NAME = "tam-research-data"
RESULT_PATH = "/vol/aera-v26/issue545-end-to-end-systems-successor1/result.json"
MAX_GPU_SECONDS = 600
RESEARCH_ISSUE = 545
SOURCE_MAIN = "8db5422929f66f89f323f2fc518c1d6b9b224581"
SOURCE_TREE = "bb20291652c5a60ac99e16d6e4ba0e53ffa27475"
...
ISSUE541_HEAD = "37599439e7d0dcdb26d45a1e04bedb6b1ec18a14"
ISSUE541_CPU_RUN = 33683772449
ISSUE541_CPU_JOB = 100426223779
ISSUE539_LAUNCHER_BLOB = "4663d86e9dc2f8eedb68213cf673ce1f80e15574"
ISSUE539_WORKFLOW_BLOB = "4e03f6e42c7635f4779f17aaa8d30488d968bc1f"
ISSUE539_TRIGGER_ISSUES = (542, 543, 544)
ISSUE539_RUNS = (33684268881, 33684274250, 33684290786)
ISSUE539_FIRST_JOB = 100427851578
ISSUE539_THIRD_JOB = 100428014282
...
PREFLIGHT_MARKER = "AERA_V26_6_ISSUE545_END_TO_END_SYSTEMS_SUCCESSOR1_PREFLIGHT_JSON="
L4_START_MARKER = "AERA_V26_6_ISSUE545_END_TO_END_SYSTEMS_SUCCESSOR1_L4_START_JSON="
RESULT_MARKER = "AERA_V26_6_ISSUE545_END_TO_END_SYSTEMS_SUCCESSOR1_RESULT_JSON="
SUMMARY_MARKER = "AERA_V26_6_ISSUE545_END_TO_END_SYSTEMS_SUCCESSOR1_SUMMARY_JSON="
...
def preflight() -> dict:
    ...
    if result_path.exists():
        raise RuntimeError(
            f"refusing duplicate issue545 successor1 systems run because result exists: {RESULT_PATH}"
        )
    ...
    if blobs != expected:
        raise RuntimeError(f"issue545 frozen blob drift: got={blobs} expected={expected}")
    ...
    for key, expected_value in required.items():
        if protocol.get(key) != expected_value:
            raise RuntimeError(
                f"issue545 evaluator protocol drift: {key}={protocol.get(key)!r} expected={expected_value!r}"
            )

    if base.CHECKPOINT_RELATIVE_DIR != "/vol/aera-real-language/v25-dev-seed8471":
        raise RuntimeError("issue545 checkpoint path drift")
    hashes = base.checkpoint_hashes(base.CHECKPOINT_RELATIVE_DIR)
    if set(hashes) != CHECKPOINT_HASH_KEYS:
        raise RuntimeError(
            f"issue545 checkpoint hash inventory drift: got={sorted(hashes)} expected={sorted(CHECKPOINT_HASH_KEYS)}"
        )
    if not all(_valid_sha256(value) for value in hashes.values()):
        raise RuntimeError("issue545 checkpoint hash value drift")

    contract = evaluator.cpu_contract_preflight_issue530()
    if contract["gpu_authorized_by_cpu_preflight"] is not False:
        raise RuntimeError("issue545 inherited #530 contract unexpectedly authorizes GPU")

    historical_issue539_result = "/vol/aera-v26/issue539-end-to-end-systems/result.json"
    if historical_issue539_result == RESULT_PATH:
        raise RuntimeError("issue545 must not reuse the historical #539 result namespace")
    ...
    return {
        ...
        "issue541_head": ISSUE541_HEAD,
        "issue541_cpu_run": ISSUE541_CPU_RUN,
        "issue541_cpu_job": ISSUE541_CPU_JOB,
        "issue539_launcher_blob": ISSUE539_LAUNCHER_BLOB,
        "issue539_workflow_blob": ISSUE539_WORKFLOW_BLOB,
        "issue539_trigger_issues": list(ISSUE539_TRIGGER_ISSUES),
        "issue539_runs": list(ISSUE539_RUNS),
        "issue539_modal_or_l4_executed": False,
        "issue527_trigger": ISSUE527_TRIGGER,
        "issue527_run": ISSUE527_RUN,
        "issue527_job": ISSUE527_JOB,
        "issue527_result_sha256": ISSUE527_RESULT_SHA256,
...
def run_gate() -> dict:
    ...
    result["issue545_successor1_gate_metadata"] = {
        "source_main": SOURCE_MAIN,
        "source_tree": SOURCE_TREE,
        "issue530_evaluator_blob": ISSUE530_EVALUATOR_BLOB,
        "v26_6_candidate_blob": V26_6_CANDIDATE_BLOB,
