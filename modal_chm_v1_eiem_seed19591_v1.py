...
# Current Modal hourly prices are re-read by the trigger workflow immediately
# before any allocation. These reference rates are only a launcher-side
# fail-closed sanity bound and are not treated as immutable pricing.
L4_USD_PER_HOUR = 0.80000
PHYSICAL_CPU_USD_PER_CORE_HOUR = 0.04730
RAM_USD_PER_GIB_HOUR = 0.00800

# Frozen evaluator identities inherited unchanged from #854 and explicitly
# retained by fresh replication preregistration #959.
...
APP_NAME = "chm-v1-eiem-optimized-seed-19591-v1"
...
def _validate_source(source_sha: str, source_tree: str, harness_sha: str) -> tuple[str, str, str]:
    ...
    if SCIENTIFIC_SEED != 19_591:
        raise RuntimeError("this launcher is bound only to scientific seed 19591")
    if RESEARCH_ISSUE != 959 or RUN_CONTROL_ISSUE != 966:
        raise RuntimeError("#959/#966 authority binding drift")
    if AUTHORIZATION_COMMENT_ID != 5_655_031_401:
        raise RuntimeError("owner authorization binding drift")
    if GPU_CLASS != "L4" or CPU_CORES != 4 or RAM_MIB != 8192 or MAX_SECONDS_PER_SEED != 3600:
        raise RuntimeError("authorized seed-19591 resource envelope drift")
    if MAX_BILLED_COMPUTE_USD != 1.25:
        raise RuntimeError("authorized seed-19591 budget cap drift")
    if (
        VALIDATION_BATCHES != 64
        ...
        raise RuntimeError("frozen evaluation envelope drift")
    if _worst_case_one_seed_cost_usd() > MAX_BILLED_COMPUTE_USD:
        raise RuntimeError("reference-rate one-seed envelope exceeds authorized $1.25 cap")
...
def _execution_manifest() -> dict[str, Any]:
    return {
        ...
        "scientific_seed_order": [19_591, 19_592, 19_593],
        "authorized_seed_only": 19_591,
        ...
        "resource_plan": {
            ...
            "max_seconds": MAX_SECONDS_PER_SEED,
        ...
        "evaluation_plan": {
            ...
            "timing_batch_size": TIMING_THROUGHPUT_BATCH_SIZE,
        },
    }
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    from tam_research.chm_v1_optimized_replication import (
        replication_preflight,
        validate_replication_seed,
    )
    ...
    validate_replication_seed(SCIENTIFIC_SEED, paid_run_authorized=True)
    volume.reload()
    root = Path(RESULT_ROOT)
    if root.exists():
        raise RuntimeError("reserved seed-19591 result namespace already exists; fail closed")

    inherited_protocol = protocol_preflight(DATA_DIR)
    replication = replication_preflight()
    ...
    payload = {
        "status": "PASS",
        "classification": "CHM_V1_ZERO_GPU_PREFLIGHT_SEED_19591_V1",
        "phase": PHASE,
        ...
        "inherited_training_protocol": inherited_protocol,
        "replication_preflight": replication,
...
def reserve_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if marker_path.exists() or consumed_path.exists() or result_path.exists() or failure_path.exists():
        raise RuntimeError("seed-19591 trigger/result namespace was already used")

    marker = {
        "status": "DISPATCH_RESERVED",
        "classification": "CHM_V1_DURABLE_PRE_ALLOCATION_MARKER_SEED_19591_V1",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "execution_code_sha": source,
        "execution_tree_sha": tree,
...
def _run_eiem_batch(
    ...
    *,
    ...
    state_cls = OptimizedEpisodicState if mode == "indexed" else EpisodicState
    states = [state_cls(f"{session_prefix}-{i}") for i in range(x.shape[0])]
    ...
    if mode == "indexed":
        total["wrapper_seconds"] += sum(
            float(state.optimized_wrapper_seconds_total) for state in states
        )
        total["sidecar_build_seconds"] += sum(
            float(state.optimized_sidecar_build_seconds_total) for state in states
        )
...
def _ordinary_indexed_exactness_audit(model: Any, val: Any, device: Any) -> dict[str, Any]:
    ...
    with torch.no_grad():
        ...
        indexed_logits, indexed_stats = _run_eiem_batch(
            ...
            session_prefix="ordinary-exactness-optimized",
        )
    torch.cuda.synchronize(device)
    ...
    return {
        ...
        "flat_nll": flat_nll,
        "optimized_indexed_nll": indexed_nll,
        "nll_delta": indexed_nll - flat_nll,
        "indexed_verification_seconds": float(indexed_stats["verification_seconds"]),
        "optimized_index_build_seconds": float(indexed_stats["index_build_seconds"]),
        "optimized_sidecar_build_seconds": float(indexed_stats["sidecar_build_seconds"]),
        "optimized_search_seconds": float(indexed_stats["search_seconds"]),
        "optimized_wrapper_seconds": float(indexed_stats["wrapper_seconds"]),
...
def _evaluate_probes(local: Any, eiem: Any, device: Any) -> dict[str, Any]:
    ...
    with torch.no_grad():
        for probe in probes:
            ...
            for mode in ("flat", "indexed"):
                state = (
                    OptimizedEpisodicState(f"opt-{probe.family}-{probe.case_id}")
                    if mode == "indexed"
                    else EpisodicState(f"flat-{probe.family}-{probe.case_id}")
                )
                last_logits = None
                ...
                if mode == "indexed":
                    indexed_totals["wrapper_seconds"] += float(
                        state.optimized_wrapper_seconds_total
                    )
                    indexed_totals["sidecar_build_seconds"] += float(
                        state.optimized_sidecar_build_seconds_total
                    )
            ...
            if predictions["flat"] != predictions["indexed"]:
                raise AssertionError(
                    f"flat/optimized candidate prediction mismatch for {probe.family}/{probe.case_id}"
                )
    ...
    return {
        ...
        "indexed": {
            ...
            "sidecar_build_seconds": indexed_totals["sidecar_build_seconds"],
            ...
            "wrapper_seconds": indexed_totals["wrapper_seconds"],
...
def _time_eiem_inputs(model: Any, inputs: list[Any], device: Any, *, mode: str) -> dict[str, float]:
    import torch
    ...
    return {
        ...
        "sidecar_build_seconds": total["sidecar_build_seconds"],
        "search_seconds": total["search_seconds"],
        "write_seconds": total["write_seconds"],
        "wrapper_seconds": total["wrapper_seconds"],
...
def _systems_timing(local: Any, eiem: Any, val: Any, device: Any) -> dict[str, Any]:
    ...
    if sum(int(x.numel()) for x in batch1_inputs) != TIMING_BATCH1_SESSIONS * SESSION_TOKENS:
        raise RuntimeError("batch-1 timing token envelope drift")
    expected_throughput = TIMING_THROUGHPUT_BATCHES * TIMING_THROUGHPUT_BATCH_SIZE * SESSION_TOKENS
    if sum(int(x.numel()) for x in throughput_inputs) != expected_throughput:
    ...
    return {
        ...
        "batch1": {
            ...
            "eiem_optimized_indexed": _time_eiem_inputs(eiem, batch1_inputs, device, mode="indexed"),
        },
        "throughput": {
            ...
            "eiem_optimized_indexed": _time_eiem_inputs(eiem, throughput_inputs, device, mode="indexed"),
        },
    }
...
def run_seed_19591(source_sha: str, source_tree: str, harness_sha: str) -> str:
    import torch
    ...
    from tam_research.chm_v1_optimized_replication import (
        evaluate_eiem_optimized_language,
        validate_replication_seed,
    )
    ...
    if consumed_path.exists() or result_path.exists() or failure_path.exists():
        raise RuntimeError("scientific seed 19591 is already consumed")
    ...
    # Reaching this GPU function means the L4 allocation has begun. Persist the
    # consumed marker before training so a later runtime failure cannot be
    # mistaken for an unused scientific seed.
    consumed = {
        "status": "SCIENTIFIC_SEED_CONSUMED",
        "classification": "CHM_V1_L4_ALLOCATION_STARTED_SEED_19591_V1",
        "seed": SCIENTIFIC_SEED,
        "execution_code_sha": source,
        "execution_tree_sha": tree,
        "harness_blob_sha": harness,
    ...
    try:
        ...
        local, eiem = build_scientific_pair(SCIENTIFIC_SEED, device, paid_run_authorized=True)
        ...
        eiem_optimized_language = evaluate_eiem_optimized_language(
            eiem,
            val_data,
            batches=VALIDATION_BATCHES,
            batch_size=VALIDATION_BATCH_SIZE,
        ...
        for label, payload in (
            ...
            ("EIEM-FLAT", eiem_flat_language),
            ("EIEM-OPT", eiem_optimized_language),
        ):
            if int(payload["tokens_evaluated"]) != 524_288:
                raise RuntimeError(f"{label} validation token count drift")
        ...
        combined_exact_calls = float(indexed["calls"]) + float(ordinary_exactness["retrieval_calls"])
        combined_exact_matches = float(indexed["exact_matches"]) + float(ordinary_exactness["exact_matches"])
        combined_exact_rate = combined_exact_matches / max(combined_exact_calls, 1.0)
        optimized_nll_delta = float(eiem_optimized_language["nll"]) - float(eiem_flat_language["nll"])
        exactness_audit_passed = (
            combined_exact_rate == 1.0
            and bool(ordinary_exactness["logits_within_tolerance"])
            and abs(optimized_nll_delta) <= 1e-5
        )
        if exactness_audit_passed:
            systems_timing: dict[str, Any] = _systems_timing(local, eiem, val_data, device)
        else:
            systems_timing = {
                "status": "SKIPPED_OPTIMIZED_TIMING_EXACTNESS_AUDIT_FAILED",
                "seed": SYSTEMS_TIMING_SEED,
            }
        ...
        local_control_delta = float(local_control["eiem_accuracy"]) - float(local_control["local_accuracy"])
        sparse_fraction = float(indexed["indexed_reads_1024"]) / max(float(indexed["flat_reads_1024"]), 1.0)
        ...
        finite = (
            ...
            and all(
                ...
                for value in (
                    ...
                    eiem_optimized_language["nll"],
                    local_train["final_train_nll"],
                    eiem_train["final_train_nll"],
                    ordinary_exactness["flat_nll"],
                    ordinary_exactness["optimized_indexed_nll"],
                )
            )
        )
        accounting = parameter_accounting()
        numerical_or_fairness = (not finite) or (not bool(accounting["within_preregistered_one_percent"]))

        if exactness_audit_passed:
            flat_practical = float(systems_timing["batch1"]["eiem_flat"]["wall_seconds"]) + float(
                systems_timing["throughput"]["eiem_flat"]["wall_seconds"]
            )
            indexed_practical = float(
                systems_timing["batch1"]["eiem_optimized_indexed"]["wall_seconds"]
            ) + float(systems_timing["throughput"]["eiem_optimized_indexed"]["wall_seconds"])
        ...
        record = {
            ...
            "language": {
                ...
                "eiem_optimized_indexed_nll": float(eiem_optimized_language["nll"]),
                "local_perplexity": float(local_language["perplexity"]),
                "eiem_flat_perplexity": float(eiem_flat_language["perplexity"]),
                "eiem_optimized_indexed_perplexity": float(eiem_optimized_language["perplexity"]),
                "optimized_minus_flat_nll": optimized_nll_delta,
            },
            ...
            "measurements": {
                ...
                "nodes_visited": {
                    ...
                    "mean_indexed_address_reads_per_query": float(indexed["mean_indexed_reads_per_query"]),
                    "mean_flat_address_reads_per_query": float(indexed["mean_flat_reads_per_query"]),
                },
                "index_build_update_write_search_wrapper_time": {
                    "index_build_seconds": float(indexed["index_build_seconds"]),
                    "optimized_sidecar_build_seconds": float(indexed["sidecar_build_seconds"]),
                    "write_seconds": float(indexed["write_seconds"]),
                    "optimized_indexed_search_seconds": float(indexed["indexed_search_seconds"]),
                    "optimized_wrapper_seconds": float(indexed["wrapper_seconds"]),
                    "flat_reference_search_seconds": float(indexed["flat_reference_search_seconds"]),
                    "verification_seconds_probe_suite": float(indexed["verification_seconds"]),
                    "verification_seconds_ordinary_batch": float(ordinary_exactness["indexed_verification_seconds"]),
                },
                "ordinary_indexed_exactness_audit": ordinary_exactness,
                "full_ordinary_optimized_indexed": eiem_optimized_language,
        ...
        if record["indexed"]["exact_match_rate"] != 1.0:
            immediate_stop_reasons.append("optimized_indexed_flat_mismatch")
        if not bool(ordinary_exactness["logits_within_tolerance"]):
            immediate_stop_reasons.append("optimized_indexed_flat_logit_mismatch")
        if abs(optimized_nll_delta) > 1e-5:
            immediate_stop_reasons.append("optimized_indexed_flat_nll_mismatch")
        ...
        result = {
            "status": "COMPLETE",
            "classification": (
                "SEED_19591_STOP_BEFORE_19592"
                if immediate_stop_reasons
                else "SEED_19591_COMPLETE_REVIEW_BEFORE_19592"
            ),
            "phase": PHASE,
            "trigger_title": TRIGGER_TITLE,
            "research_issue": RESEARCH_ISSUE,
            ...
            "derived": {
                ...
                "ordinary_max_abs_logit_delta": float(ordinary_exactness["max_abs_logit_delta"]),
                "optimized_indexed_practical_seconds": indexed_practical,
            ...
            "interpretation_ceiling": (
                "single fresh preregistered replication seed only; no full-replication, novelty, SOTA, "
                "scaling, AGI, RSI, or breakthrough claim"
            ),
        }
...
def main(phase: str, source_sha: str, source_tree: str, harness_sha: str) -> None:
    if phase == "preflight":
        ...
        payload = run_seed_19591.remote(source_sha, source_tree, harness_sha)
