...
APP_NAME = "chm-v1-100m-stage-c-payload-gate-1017-v4"
...
def _validate_account_binding(selected_account: str, selected_workspace_name: str) -> dict[str, str]:
    account = str(selected_account).strip().lower()
    workspace = str(selected_workspace_name).strip()
    if account not in {"primary", "secondary"}:
        raise RuntimeError("#1017 selected Modal account must be primary or secondary")
    if not workspace:
        raise RuntimeError("#1017 selected Modal workspace must be non-empty")
    return {
...
def _evidence(
    ...
    account = _validate_account_binding(selected_account, selected_workspace_name)
    return {
        "phase": PHASE,
...
def inspect_source(
    ...
    selected_account: str, selected_workspace_name: str,
) -> str:
    b = _validate_bindings(source_sha, source_tree, model_sha, evaluator_sha, payload_gate_sha, scientific_impl_sha, runner_sha, workflow_sha)
    volume.reload()
    state = _inspect_source()
    return json.dumps({
        "classification": "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_SOURCE_AUDIT",
        **_evidence(b, 0, selected_account, selected_workspace_name), **state,
...
def verify_zero_gpu(
    source_sha: str, source_tree: str, model_sha: str, evaluator_sha: str,
    payload_gate_sha: str, scientific_impl_sha: str, runner_sha: str, workflow_sha: str,
    authority_comment_id: int, selected_account: str, selected_workspace_name: str,
) -> str:
    b = _validate_bindings(source_sha, source_tree, model_sha, evaluator_sha, payload_gate_sha, scientific_impl_sha, runner_sha, workflow_sha)
    ...
    gate = {
        ...
        **_evidence(b, authority_comment_id, selected_account, selected_workspace_name), **state,
...
def reserve_dispatch(
    ...
    expected = _evidence(b, authority_comment_id, selected_account, selected_workspace_name)
...
def run_diagnostic(
    ...
    evidence = _evidence(b, authority, selected_account, selected_workspace_name)
    consumed = {
...
def inspect_state(
    ...
    return json.dumps({
        **_evidence(b, authority_comment_id, selected_account, selected_workspace_name),
...
def main(
    ...
    base = (source_sha, source_tree, model_sha, evaluator_sha, payload_gate_sha, scientific_impl_sha, runner_sha, workflow_sha)
    if phase == "inspect-source":
        print(
            f"CHM_V1_100M_STAGE_C_PAYLOAD_GATE_SOURCE="
            f"{inspect_source.remote(*base, selected_account, selected_workspace_name)}"
        )
        return
    common = (
        *base,
