...
APP_NAME = "tam-research-aera-v26-3-issue449-ficem-read-repair4"
VOLUME_NAME = "tam-research-data"
RESULT_PATH = "/vol/aera-v26/issue449-ficem-read-repair4/result.json"
MAX_GPU_SECONDS = 300
FROZEN_PROBE_GIT_BLOB = "16aa99b9f6f0a1d11bd7bf5f36b2f0b1fb97047b"
MERGED_REPAIR4_BACKEND_GIT_BLOB = "a3a603c8a2d4b20ebcccd7663970978f4288a760"
SOURCE_CPU_PR = 446
SOURCE_CPU_RUN = 33535610189
SOURCE_CPU_JOB = 99949079275
SOURCE_CPU_HEAD = "4081eff33c754113b4df7efd39bf1a6f04a9556a"
SOURCE_MERGED_MAIN = "33ec518f978cc95afe5165fc3b3b054151a2475c"
...
def _summary(result: dict) -> dict:
    ...
    return {
        ...
        "candidate_no_reference_tail_ops_pass": result["candidate_no_reference_tail_ops_pass"],
...
def preflight() -> dict:
    ...
    if result_path.exists():
        raise RuntimeError(f"refusing duplicate issue449 FICEM read repair4 run because result exists: {RESULT_PATH}")
    ...
    if probe_blob != FROZEN_PROBE_GIT_BLOB:
        raise RuntimeError(f"issue449 frozen probe blob drifted: {probe_blob}")
    if backend_blob != MERGED_REPAIR4_BACKEND_GIT_BLOB:
        raise RuntimeError(f"issue449 merged repair4 backend blob drifted: {backend_blob}")
    ...
    for key, value in required.items():
        if protocol.get(key) is not value:
            raise RuntimeError(f"issue449 protocol boundary drifted: {key}={protocol.get(key)!r}")

    check = {
        "contract": cpu_contract_preflight(),
        "research_issue": 449,
        "source_repair_issue": 445,
    ...
    print("AERA_V26_ISSUE449_FICEM_READ_REPAIR4_PREFLIGHT_JSON=" + json.dumps(check, separators=(",", ":")), flush=True)
    return check
...
def run_probe() -> dict:
    ...
    print(
        "AERA_V26_ISSUE449_FICEM_READ_REPAIR4_L4_START_JSON="
        + json.dumps(
            {
    ...
    if not all(bool(localized[key]) for key in required_localized):
        raise RuntimeError("issue449 bfloat16_batch8_mixed did not pass all frozen repair4 correctness subgates")

    result_path.parent.mkdir(parents=True, exist_ok=True)
    result_path.write_text(json.dumps(result, indent=2))
    volume.commit()
    print("AERA_V26_ISSUE449_FICEM_READ_REPAIR4_RESULT_JSON=" + json.dumps(result, separators=(",", ":")), flush=True)
    print("AERA_V26_ISSUE449_FICEM_READ_REPAIR4_SUMMARY_JSON=" + json.dumps(_summary(result), separators=(",", ":")), flush=True)
    return result
...
def main() -> None:
    ...
    print("AERA_V26_ISSUE449_FICEM_READ_REPAIR4_PREFLIGHT_LOCAL=" + json.dumps(check, separators=(",", ":")), flush=True)
