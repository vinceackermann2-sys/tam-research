...
PHASE = "host-sync-profile-v3"
TRIGGER_TITLE = "[modal-cortex-s-100m-host-sync-profile-v3]"
ENGINEERING_SEED = 2_026_091_007
...
APP_NAME = "cortex-s-v0-100m-host-sync-profile-v3"
...
def _validate_payload(source_sha: str, source_tree: str, harness_sha: str) -> tuple[str, str, str]:
    ...
    if ENGINEERING_SEED in forbidden:
        raise RuntimeError("host-sync v3 engineering seed is retired, consumed, or scientifically reserved")
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if root.exists():
        raise RuntimeError("reserved host-sync attribution v3 namespace already exists; fail closed")
    ...
    payload = {
        "status": "PASS",
        "classification": "ZERO_GPU_HOST_SYNC_ATTRIBUTION_V3_GATE",
        "phase": PHASE,
...
def reserve_h100_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if not zero_path.exists():
        raise RuntimeError("zero-GPU attribution v3 gate is missing")
    ...
    for key, value in {"source_sha": source, "source_tree": tree, "harness_sha": harness, "status": "PASS"}.items():
        if zero.get(key) != value:
            raise RuntimeError(f"zero-GPU attribution v3 gate mismatch for {key}")
    if marker_path.exists() or result_path.exists():
        raise RuntimeError("host-sync attribution v3 engineering attempt is already consumed")

    marker = {
        "status": "H100_DISPATCH_CONSUMED",
        "classification": "ENGINEERING_SINGLE_ATTEMPT_MARKER_HOST_SYNC_ATTRIBUTION_V3",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "source_sha": source,
        "source_tree": tree,
...
def h100_profile(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    from architectures.cortex_s.host_sync_profile_v3 import run_host_sync_profile
    from architectures.cortex_s.production_scan_integration_v1 import build_memory_lean_grouped_triton_scan_cortex_100m
    ...
    if result_path.exists():
        raise RuntimeError("host-sync attribution v3 result already exists; refusing rerun")
