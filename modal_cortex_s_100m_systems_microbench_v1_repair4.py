...
# GitHub credentials never enter Modal. The remote side only writes its immutable,
# single-use evidence namespace; GitHub Actions owns issue comments and source binding.
image = (
...
def _json_safe(value):
    """Force remote return values to stdlib JSON types.

    Repair3's H100 result was already persisted successfully, but the GitHub runner
    failed while unpickling a torch-owned string subclass because local torch was not
    installed. The launcher should not need the heavyweight research runtime merely
    to receive evidence, so repair4 normalizes the payload before returning it.
    """
...
def verify_data_zero_gpu(source_sha: str) -> dict:
    ...
    if REMOTE_ENGINEERING_SEED != ENGINEERING_SEED:
        raise RuntimeError("repair4 launcher/remote engineering seed drift")
    ...
    if protocol.get("full_training_authorized") is not False or protocol.get("next_stage_authorized") is not False:
        raise RuntimeError("repair4 protocol unexpectedly authorizes a later stage")
    ...
    if data_path.exists() or dispatch_path.exists() or result_path.exists():
        raise RuntimeError("systems microbenchmark repair4 namespace is already consumed")

    data = _verify_frozen_data()
    contract = zero_gpu_physical_padding_contract_probe()
    if contract.get("status") != "PASS":
        raise RuntimeError("repair4 zero-GPU physical-padding contract did not pass")
    result = {
        "status": "PASS",
        "scientific_status": "ZERO_GPU_SYSTEMS_DATA_AND_PHYSICAL_PADDING_GATE_ONLY",
        "repair_namespace": "microbench-v1-repair4",
        "source_sha": source_sha,
        "engineering_seed": ENGINEERING_SEED,
        "data": data,
        "physical_padding_contract": contract,
...
def h100_microbenchmark(source_sha: str) -> dict:
    """Exactly one bounded repair4 H100 systems measurement; never 2B training."""
    ...
    if not data_path.exists():
        raise RuntimeError("repair4 zero-GPU data/physical-padding gate is missing")
    ...
    if data_gate.get("status") != "PASS" or data_gate.get("source_sha") != source_sha:
        raise RuntimeError("repair4 zero-GPU gate does not authorize this exact source SHA")
    if dispatch_path.exists() or result_path.exists():
        raise RuntimeError("systems H100 microbenchmark repair4 namespace already consumed")
    ...
    try:
        result = _json_safe(run_h100_systems_microbenchmark(source_sha=source_sha, data_dir=DATA_DIR))
    ...
    result["repair_namespace"] = "microbench-v1-repair4"
