...
APP_NAME = "aera-v26-9-issue650-e2e-cli-guard-continuation-l4"
VOLUME_NAME = "tam-research-data"
RESULT_PATH = "/vol/aera-v26/issue650-e2e-cli-guard-continuation/result.json"
MAX_GPU_SECONDS = 600

RESEARCH_ISSUE = 643
CONTINUATION_ISSUE = 650
BOUND_SOURCE_MAIN = "eb2cbab526140b6352cb4d09a06c48b8bbd80fc5"
BOUND_SOURCE_TREE = "6b1cb0b65ac51e4bbc14e4c01620634df97f3979"
...
ISSUE649_TRIGGER = 649
ISSUE649_RUN = 34021311045
ISSUE649_JOB = 101454307178
...
PRECHECK_MARKER = "AERA_V26_9_ISSUE650_E2E_CLI_GUARD_CONTINUATION_PRECHECK_JSON="
PREFLIGHT_MARKER = "AERA_V26_9_ISSUE650_E2E_CLI_GUARD_CONTINUATION_PREFLIGHT_JSON="
L4_START_MARKER = "AERA_V26_9_ISSUE650_E2E_CLI_GUARD_CONTINUATION_L4_START_JSON="
RESULT_MARKER = "AERA_V26_9_ISSUE650_E2E_CLI_GUARD_CONTINUATION_RESULT_JSON="
SUMMARY_MARKER = "AERA_V26_9_ISSUE650_E2E_CLI_GUARD_CONTINUATION_SUMMARY_JSON="
...
def _summary(result: dict) -> dict:
    ...
    return {
        ...
        "issue650_continuation_metadata": result.get(
            "issue650_continuation_metadata", {}
        ),
    }
...
def preflight() -> dict:
    ...
    if result_path.exists():
        raise RuntimeError(
            f"refusing duplicate issue650 continuation because result exists: {RESULT_PATH}"
        )
    ...
    if blobs != expected:
        raise RuntimeError(
            f"issue650 frozen scientific blob drift: got={blobs} expected={expected}"
        )
    ...
    for key in (
        ...
        if contract[key] is not False:
            raise RuntimeError(f"issue650 inherited #643 CPU contract drifted: {key}")

    historical571 = Path(ISSUE571_RESULT_PATH)
    if not historical571.exists():
        raise RuntimeError("issue650 immutable #571 result is missing")
    if _sha256_file(historical571) != ISSUE571_RESULT_SHA256:
        raise RuntimeError("issue650 immutable #571 result SHA drifted")
    ...
    if (
        ...
        raise RuntimeError("issue650 immutable #571 decision drifted")

    primitive630 = Path(ISSUE630_RESULT_PATH)
    if not primitive630.exists():
        raise RuntimeError("issue650 immutable #630 result is missing")
    if _sha256_file(primitive630) != ISSUE630_RESULT_SHA256:
        raise RuntimeError("issue650 immutable #630 result SHA drifted")
    ...
    if (
        ...
        raise RuntimeError("issue650 immutable #630 decision drifted")

    if base.CHECKPOINT_RELATIVE_DIR != "/vol/aera-real-language/v25-dev-seed8471":
        raise RuntimeError("issue650 checkpoint path drift")
    hashes = base.checkpoint_hashes(base.CHECKPOINT_RELATIVE_DIR)
    if hashes != CHECKPOINT_HASHES:
        raise RuntimeError(
            f"issue650 checkpoint hash drift: got={hashes} expected={CHECKPOINT_HASHES}"
        )
    ...
    return {
        ...
        "issue649_trigger": ISSUE649_TRIGGER,
        "issue649_run": ISSUE649_RUN,
        "issue649_job": ISSUE649_JOB,
...
def run_gate() -> dict:
    ...
    if device != "NVIDIA L4":
        raise RuntimeError(f"issue650 requires exact NVIDIA L4, got {device!r}")
        ...
        + json.dumps(
            {
                ...
                "issue649_consumed_pre_modal": True,
                "scientific_seed_consumed": False,
                "architecture_freeze_authorized": False,
                "s2_authorized": False,
                "fresh_scientific_seed_authorized": False,
    ...
    result["issue650_continuation_metadata"] = {
        "continuation_issue": CONTINUATION_ISSUE,
        "bound_source_main": BOUND_SOURCE_MAIN,
        "bound_source_tree": BOUND_SOURCE_TREE,
        ...
        "issue649_job": ISSUE649_JOB,
        "issue649_consumed_pre_modal": True,
        "issue649_guard_error": "accepts 1 arg(s), received 4",
        "issue643_adapter_blob": ISSUE643_ADAPTER_BLOB,
