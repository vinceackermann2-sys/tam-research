...
"""CPU-only delayed-associative necessity diagnostic for AERA-v20.

This deliberately reuses the frozen v19/#278 task family, optimizer, training
budget, evaluator and success thresholds.  The only experimental variable is the
model constructor: v20 factorized address/payload writes instead of v19's single
summary write.  No synthetic address/value auxiliary is used here.
"""
...
def build_model(seed: int) -> HardwareAwareAERATextLMV20:
    torch.manual_seed(seed)
    model = HardwareAwareAERATextLMV20(diagnostic_config())
    _force_all_stages_run(model)
    return model


def train_pair(*, steps: int = TRAIN_STEPS) -> tuple[HardwareAwareAERATextLMV20, HardwareAwareAERATextLMV20, dict[str, Any]]:
    ...
    for step in range(steps):
        ...
        if step in {0, steps // 4, steps // 2, (3 * steps) // 4, steps - 1}:
            ...
            history.append(row)
            print(
                "AERA_V20_MEMORY_NECESSITY_TRAIN=" + json.dumps(row, sort_keys=True),
                flush=True,
            )
...
def run_diagnostic(*, steps: int = TRAIN_STEPS) -> dict[str, Any]:
    ...
    same_checkpoint_memory_contribution = (
        full_eval["query_accuracy"] - full_memory_off["query_accuracy"]
    )
    ...
    checks = {
        ...
        "same_checkpoint_memory_drop_ge_0_15": (
            same_checkpoint_memory_contribution >= SAME_CHECKPOINT_MEMORY_DROP_MIN
        ),
        "overwrite_current_value_accuracy_ge_0_80": (
            full_eval["overwrite_current_value_accuracy"] >= OVERWRITE_ACCURACY_MIN
        ),
        "stale_value_error_le_0_10": full_eval["stale_value_error_rate"] <= STALE_ERROR_MAX,
        "fresh_session_near_chance": (
            safety["fresh_session_query_accuracy"]
            <= chance + FRESH_SESSION_CHANCE_TOLERANCE
        ),
        "deployment_base_parameters_unchanged": safety[
    ...
    result: dict[str, Any] = {
        "version": "aera-v20-factorized-write-memory-necessity-cpu",
        "task": {
            "oracle_accuracy": validity,
            "chance_accuracy": chance,
            "frozen_source": "aera_v19_memory_necessity_cpu/#278",
        },
        "full_stream_plus_memory": {
            k: v for k, v in full_eval.items() if k != "predictions"
        },
        "same_checkpoint_memory_disabled": {
            k: v for k, v in full_memory_off.items() if k != "predictions"
        },
        ...
        "same_checkpoint_memory_contribution_accuracy": same_checkpoint_memory_contribution,
...
def main() -> None:
    ...
    print(
        "AERA_V20_MEMORY_NECESSITY_RESULT_JSON=" + json.dumps(result, sort_keys=True),
        flush=True,
    )
