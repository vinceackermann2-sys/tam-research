...
# This file is inert until a separately authorized exact-title issue trigger exists.
...
APP_NAME = "cortex-s-v0-100m-2b-preflight-v6-host-sync-coalesced"
...
def _validate_payload(
    ...
    if ENGINEERING_SEED in forbidden:
        raise RuntimeError("v6 engineering seed is retired, consumed, or scientifically reserved")
...
def _verify_coalesced_training_step(training_module: Any) -> dict[str, Any]:
    ...
    if step_source.count('.to(device="cpu")') != 1:
        raise RuntimeError("optimizer-step host readback count drifted from exactly one")
...
def _verify_frozen_inputs() -> dict[str, Any]:
    ...
    hashes["training_step_contract"] = _verify_coalesced_training_step(training_module)
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if root.exists():
        raise RuntimeError("reserved v6 result namespace already exists; fail closed")
    ...
    payload = {
        "status": "PASS",
        "classification": "ZERO_GPU_HOST_SYNC_COALESCED_PREFLIGHT_V6",
        "phase": PHASE,
...
def reserve_h100_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if marker_path.exists() or result_path.exists():
        raise RuntimeError("v6 engineering attempt is already consumed")
    marker = {
        "status": "H100_DISPATCH_CONSUMED",
        "classification": "ENGINEERING_SINGLE_ATTEMPT_MARKER_V6_HOST_SYNC_COALESCED",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "source_sha": source,
        "source_tree": tree,
...
def h100_preflight(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if result_path.exists():
        raise RuntimeError("v6 RESULT.json already exists; refusing rerun")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("v6 systems preflight requires CUDA")
    device = torch.device("cuda")
    if torch.cuda.get_device_capability(device)[0] < 9:
        raise RuntimeError("v6 systems preflight requires H100/SM90-class CUDA")
    ...
    if _sha256(_training_module_and_path()[1]) != TRAIN_IMPLEMENTATION_SHA256:
        raise RuntimeError("coalesced training implementation changed before H100 execution")
    _verify_coalesced_training_step(training_module)
    ...
    try:
        ...
        try:
            with memory_lean_grouped_triton_scan_training_builder():
                raw = training_module.run_h100_calibration(
                    data_dir=DATA_DIR,
                    output_path="/tmp/cortex_s_v6_host_sync_coalesced_calibration.json",
                    source_sha=source,
                )
        finally:
        ...
        status = "SYSTEMS_PREFLIGHT_PASS" if calibration_pass and dispatcher_pass else "SYSTEMS_PREFLIGHT_FAIL"
        result = {
            "status": status,
            "classification": "ENGINEERING_SYSTEMS_PREFLIGHT_V6_HOST_SYNC_COALESCED_ONLY",
            "phase": PHASE,
            "source_sha": source,
            "source_tree": tree,
            "harness_sha": harness,
            ...
            "training_step_contract": _verify_coalesced_training_step(training_module),
            "calibration": {
        ...
        failure = {
            ...
            "classification": "ENGINEERING_SYSTEMS_PREFLIGHT_V6_HOST_SYNC_COALESCED_ERROR",
            "phase": PHASE,
            "source_sha": source,
            "source_tree": tree,
            "harness_sha": harness,
