...
def test_pair1_runner_freezes_identity_geometry_and_authority() -> None:
    ...
    assert 'REQUIRED_MODAL_ACCOUNT = "secondary"' in source
    assert 'PREDECESSOR_ROOT = "/vol/cortex-s-v0/100m-2b/reduced-attention-pair1-v1"' in source
    assert "def _verify_predecessor()" in source
    assert '"predecessor_pair_seed_consumed": 58_231' in source


def test_successor_requires_frozen_incomplete_predecessor_evidence() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    predecessor = _function(source, "_verify_predecessor")
    assert 'PREDECESSOR_ROOT' in predecessor
    assert 'reservation.get("pair_seed") != 58_231' in predecessor
    assert 'reservation.get("pair_seed_consumed") is not True' in predecessor
    assert 'transformer.get("status") != "COMPLETE"' in predecessor
    assert 'reduced.get("status") != "ERROR"' in predecessor
    assert 'SCIENTIFIC_REDUCED_ATTENTION_PAIR1_ARCHITECTURE_ERROR_NO_RETRY' in predecessor
    assert 'reduced.get("automatic_retry_authorized") is not False' in predecessor
    assert '"successor_is_retry": False' in predecessor
...
def test_pair1_screen_is_exact_preregistered_rule() -> None:
    ...
    assert '"PAIR1_SUCCESSOR_SCREEN_PASS"' in finalizer
    assert '"PAIR1_SUCCESSOR_SCREEN_FAIL"' in finalizer
    assert '"PAIR1_SUCCESSOR_INCOMPLETE_NO_RETRY"' in finalizer
...
def test_pair1_workflow_is_exact_owner_only_one_shot_and_dual_account_bound() -> None:
    ...
    assert (
        ...
        "'[modal-cortex-reduced-attention-pair1-successor-100m-2b-v2]'"
        in source
    )
