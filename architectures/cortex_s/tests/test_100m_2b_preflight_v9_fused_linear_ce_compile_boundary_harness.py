...
def test_candidate_normalizes_generic_eager_fallback_without_changing_default_trainer() -> None:
    ...
    # Guard executable misuse rather than matching explanatory docstrings.
    assert "return self.model(tokens, targets)" not in source
    assert "loss = model(x, y)" not in source
