...
from architectures.cortex_s.host_sync_profile_v2 import (
...
def test_repo_root_resolution_is_cwd_independent() -> None:
    ...
    assert (REPO_ROOT / "modal_cortex_s_100m_host_sync_profile_v2.py").is_file()
...
def test_production_call_arithmetic_and_source_are_unchanged() -> None:
    ...
    train = _repo_text("architectures/cortex_s/experiments/scale100m_2b/train.py")
...
def test_authority_and_profiler_contract_are_static() -> None:
    core = _repo_text("architectures/cortex_s/host_sync_profile_v2.py")
    runner = _repo_text("modal_cortex_s_100m_host_sync_profile_v2.py")
    workflow = _repo_text(".github/workflows/modal-cortex-s-100m-host-sync-profile-v2.yml")

    assert 'PHASE = "host-sync-profile-v2"' in runner
    assert 'TRIGGER_TITLE = "[modal-cortex-s-100m-host-sync-profile-v2]"' in runner
    assert "ENGINEERING_SEED = 2_026_091_006" in runner
    assert 'RESULT_ROOT = "/vol/cortex-s-v0/100m-2b/host-sync-profile-v2"' in runner
    assert "2_026_091_005" in runner
    ...
    assert "modal run --detach --timestamps modal_cortex_s_100m_host_sync_profile_v2.py" in workflow
