...
APP_NAME = "cortex-s-v0-100m-full-step-profile-v2"
...
def _validate_payload(source_sha: str, source_tree: str, harness_sha: str) -> tuple[str, str, str]:
    ...
    if ENGINEERING_SEED in forbidden:
        raise RuntimeError("full-step profiler engineering seed is retired, consumed, or scientifically reserved")
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if root.exists():
        raise RuntimeError("reserved full-step profile v2 namespace already exists; fail closed")
    ...
    payload = {
        "status": "PASS",
        "classification": "ZERO_GPU_FULL_STEP_PROFILE_V2_GATE",
        "phase": PHASE,
...
def reserve_h100_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if marker_path.exists() or result_path.exists():
        raise RuntimeError("full-step profiler v2 engineering attempt is already consumed")
    marker = {
        "status": "H100_DISPATCH_CONSUMED",
        "classification": "ENGINEERING_SINGLE_ATTEMPT_MARKER_FULL_STEP_PROFILE_V2",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "source_sha": source,
        "source_tree": tree,
...
def h100_profile(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    from architectures.cortex_s.full_step_profile_v2 import run_full_step_profile
    from architectures.cortex_s.production_scan_integration_v1 import build_memory_lean_grouped_triton_scan_cortex_100m
    ...
    if result_path.exists():
        raise RuntimeError("full-step profiler v2 result already exists; refusing rerun")
    ...
    if "H100" not in gpu_name.upper() or torch.cuda.get_device_capability(device)[0] < 9:
        raise RuntimeError(f"expected H100/SM90, got {gpu_name} capability={torch.cuda.get_device_capability(device)}")
    if not triton_available():
        raise RuntimeError("Triton is unavailable on the H100 profiler image")
    ...
    payload = {
        ...
        "full_training_authorized": False,
        "scientific_claim_authorized": False,
        "note": "Diagnostic attribution only. Profiler overhead is excluded from throughput evidence; CPU and device self-time shares can overlap and are not additive wall-time shares.",
    }
