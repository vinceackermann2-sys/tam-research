...
APP_NAME = "tam-research-aera-v15-real-language-dev-seed8291"
VOLUME_NAME = "tam-research-data"
SEED = 8291
DATA_DIR = "/vol/data/tam100m-2b-curated-v1"
RUN_DIR = f"/vol/aera-real-language/v15-dev-seed{SEED}"
...
def preflight() -> dict:
    from tam_research.aera_real_language_v15_gpu import validate_protocol

    volume.reload()
    result = validate_protocol(DATA_DIR)
    print("AERA_V15_DEV_PREFLIGHT_JSON=" + json.dumps(result, separators=(",", ":")), flush=True)
    return result
...
def run_gate() -> dict:
    ...
    from tam_research.aera_real_language_v15_gpu import train_matched_pair
    ...
    volume.commit()
    print("AERA_V15_DEV_RESULT_JSON=" + json.dumps(result, separators=(",", ":")), flush=True)
    return result
...
def main() -> None:
    ...
    print("AERA_V15_DEV_PREFLIGHT_LOCAL=" + json.dumps(check, separators=(",", ":")), flush=True)
