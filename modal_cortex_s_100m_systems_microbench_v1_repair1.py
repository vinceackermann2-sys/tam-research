...
APP_NAME = "cortex-s-v0-100m-systems-microbench-v1-repair1"
VOLUME_NAME = "tam-research-data"
RESULT_ROOT = "/vol/cortex-s-v0/100m-systems-microbench-v1-repair1"
...
def verify_data_zero_gpu(
    ...
    """Re-hash the immutable corpus before repair1 can allocate the H100."""
    ...
    if protocol.get("full_training_authorized") is not False:
        raise RuntimeError("core microbenchmark protocol unexpectedly authorizes full training")
    volume.reload()
    ...
    if data_path.exists() or dispatch_path.exists() or result_path.exists():
        raise RuntimeError("systems microbenchmark repair1 namespace is already consumed")
    data = _verify_frozen_data()
    result = {
        "status": "PASS",
        "scientific_status": "ZERO_GPU_SYSTEMS_DATA_GATE_ONLY",
        "repair_namespace": "microbench-v1-repair1",
    ...
    _comment(
        repo_full_name,
        issue_number,
        "🟩 **CORTEX-S systems microbench repair1 zero-GPU gate PASS** — exact corpus hashes verified in the fresh repair namespace. No H100 allocated yet.",
    )
    return result
...
def h100_microbenchmark(
    ...
    """One single-use repair1 engineering H100 measurement; never a 2B run."""
    ...
    if not data_path.exists():
        raise RuntimeError("repair1 zero-GPU data gate is missing")
    ...
    if data_gate.get("status") != "PASS" or data_gate.get("source_sha") != source_sha:
        raise RuntimeError("repair1 zero-GPU gate does not authorize this exact source SHA")
    if dispatch_path.exists() or result_path.exists():
        raise RuntimeError("systems H100 microbenchmark repair1 namespace already consumed")
    ...
    _comment(
        repo_full_name,
        issue_number,
        "🟨 **CORTEX-S systems microbench repair1 H100 started** — one engineering seed only, <=15-minute hard timeout. No paired/scientific training function exists in this app.",
    )
    ...
    try:
        ...
        _comment(
            repo_full_name,
            issue_number,
            f"🟥 **CORTEX-S systems microbench repair1 failed safely** — {failure['error']}. Engineering seed/repair namespace are consumed; no retry and no full training.",
        )
        raise

    result = dict(result)
    result["repair_namespace"] = "microbench-v1-repair1"
    ...
    _comment(
        repo_full_name,
        issue_number,
        "🧪 **CORTEX-S systems microbench repair1 complete** — "
        f"status={result['status']}, grouped/legacy full-model speedup={speedup_text}. "
        "Engineering evidence only; this does not authorize the 2B run or another H100 dispatch.",
    )
...
def main(
    phase: str = "microbench-v1-repair1",
    source_sha: str = "",
    repo_full_name: str = "",
    issue_number: int = 0,
):
    if phase.strip().lower() != "microbench-v1-repair1":
        raise ValueError("only phase='microbench-v1-repair1' is supported")
