...
APP_NAME = "cortex-s-v0-100m-2b-preflight-v8-fused-linear-ce-packaging"
...
def _validate_payload(source_sha: str, source_tree: str, harness_sha: str) -> tuple[str, str, str]:
    ...
    if ENGINEERING_SEED in forbidden:
        raise RuntimeError("v8 engineering seed is retired, consumed, or scientifically reserved")
    if TOKENS_PER_STEP != 65_536 or MEASURED_TOKENS != 2_621_440:
        raise RuntimeError("production calibration geometry drift")
    if GLOBAL_BATCH != MICRO_BATCH_SIZE * GRAD_ACCUM_STEPS:
        raise RuntimeError("global-batch drift")
    return source, tree, harness


def _dependency_smoke() -> dict[str, Any]:
    packaging_installed = importlib.metadata.version("packaging")
    liger_installed = importlib.metadata.version("liger-kernel")
    if packaging_installed != PACKAGING_VERSION:
        raise RuntimeError(f"packaging version drift: {packaging_installed} != {PACKAGING_VERSION}")
    if liger_installed != LIGER_KERNEL_VERSION:
        raise RuntimeError(f"liger-kernel version drift: {liger_installed} != {LIGER_KERNEL_VERSION}")
    from packaging.version import Version
    from liger_kernel.transformers import LigerFusedLinearCrossEntropyLoss

    if Version(packaging_installed) != Version(PACKAGING_VERSION):
        raise RuntimeError("packaging.Version import/version proof failed")
    if LigerFusedLinearCrossEntropyLoss is None:
        raise RuntimeError("Liger fused-linear-CE import proof failed")
    return {
        "packaging_version": packaging_installed,
        "packaging_version_import_ok": True,
        "liger_kernel_version": liger_installed,
        "liger_fused_linear_ce_import_ok": True,
    }
...
def _verify_default_training_step(training_module: Any) -> dict[str, Any]:
    source = inspect.getsource(training_module._one_optimizer_step)
    for needle in (
        ...
        if needle not in source:
            raise RuntimeError(f"coalesced training-step contract missing: {needle}")
    if source.count('.to(device="cpu")') != 1:
        raise RuntimeError("production optimizer-step host readback count drift")
    if source.index("host_report = torch.stack(") > source.index("optimizer.step()"):
        raise RuntimeError("host safety readback must precede optimizer update")
    return {"return_api": "float", "intentional_device_to_cpu_readbacks": 1, "abort_before_optimizer_update": True}
...
def _verify_fused_candidate_contract() -> dict[str, Any]:
    ...
    for key, value in expected.items():
        if status.get(key) != value:
            raise RuntimeError(f"fused candidate contract drift for {key}")
    step_source = inspect.getsource(candidate._one_optimizer_step_fused_linear_ce)
    if step_source.count('.to(device="cpu")') != 1:
        raise RuntimeError("fused optimizer-step host readback count drift")
    if "runner(x, y) / training_module.GRAD_ACCUM_STEPS" not in step_source:
        raise RuntimeError("fused target/training contract drift")
    return {"systems_variant": FUSED_SYSTEMS_VARIANT, "intentional_device_to_cpu_readbacks": 1, "cuda_materializes_full_logits": False}


def _verify_frozen_inputs() -> dict[str, Any]:
    training_module, implementation_path = _training_module_and_path()
    root = Path(DATA_DIR)
    train_path, val_path, meta_path = root / "train.bin", root / "val.bin", root / "meta.json"
    if any(not p.exists() for p in (train_path, val_path, meta_path)):
        raise FileNotFoundError("frozen 100M/2B corpus missing")
    ...
    for key, expected in {
        ...
        if meta.get(key) != expected:
            raise RuntimeError(f"corpus metadata drift for {key}")
    observed = {
    ...
    for key, expected in {
        ...
        if observed[key] != expected:
            raise RuntimeError(f"frozen fingerprint mismatch for {key}")
    observed["default_training_step_contract"] = _verify_default_training_step(training_module)
    observed["fused_candidate_contract"] = _verify_fused_candidate_contract()
    return observed
...
def _model_invariants(model: Any) -> dict[str, Any]:
    ...
    grouped = sum(isinstance(b.moe, MemoryLeanPhysicalPaddedGroupedSparseMoE) for b in blocks)
    worlds = sum(isinstance(b.world, ProductionScanPersistentWorldState) for b in blocks)
    attention = sum(bool(b.has_attention) for b in blocks)
    if params != EXPECTED_CORTEX_PARAMS or grouped != EXPECTED_LAYERS or worlds != EXPECTED_LAYERS:
        raise RuntimeError("production model/backend identity drift")
    for key, expected in {
    ...
    if {int(b.moe.physical_hidden) for b in blocks} != {EXPECTED_PHYSICAL_HIDDEN}:
    ...
    if status.get("systems_variant") != INTEGRATED_SYSTEMS_VARIANT or status.get("cuda_backend") != PRODUCTION_SCAN_BACKEND:
        raise RuntimeError("production integrated backend drift")
    return {"parameter_count": params, "grouped_moe_layers": grouped, "integrated_worlds": worlds, "attention_layers": attention}
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if root.exists():
        raise RuntimeError("reserved v8 result namespace already exists; fail closed")
    dependency_smoke = _dependency_smoke()
    frozen_inputs = _verify_frozen_inputs()
    model = build_memory_lean_grouped_triton_scan_cortex_100m()
    payload = {
        "status": "PASS",
        "classification": "ZERO_GPU_FUSED_LINEAR_CE_PACKAGING_PREFLIGHT_V8",
        "phase": PHASE,
        ...
        "gpu_allocated": False,
        "dependency_smoke": dependency_smoke,
...
def reserve_h100_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if zero.get("dependency_smoke", {}).get("packaging_version") != PACKAGING_VERSION:
        raise RuntimeError("packaging dependency smoke was not durably proven")
    if not zero.get("dependency_smoke", {}).get("liger_fused_linear_ce_import_ok"):
        raise RuntimeError("Liger import smoke was not durably proven")
    if marker_path.exists() or result_path.exists():
        raise RuntimeError("v8 engineering attempt is already consumed")
    marker = {
        "status": "H100_DISPATCH_CONSUMED",
        "classification": "ENGINEERING_SINGLE_ATTEMPT_MARKER_V8_FUSED_LINEAR_CE_PACKAGING",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "source_sha": source,
        "source_tree": tree,
...
def _prove_triton_dispatch(model: Any, train_data: Any, device: Any) -> dict[str, Any]:
    ...
    original_candidate, original_fallback = integration.affine_scan_triton_candidate, integration.affine_scan
    ...
    if calls != {"candidate": EXPECTED_LAYERS, "fallback": 0}:
        raise RuntimeError(f"dispatcher proof drift: {calls}")
    return {"candidate_calls": calls["candidate"], "legacy_fallback_calls": calls["fallback"], "selected_triton_for_all_worlds": True}
...
def _prove_fused_loss_cuda(model: Any) -> dict[str, Any]:
    ...
    if runner.model is not model or not runner._use_liger or runner._liger_loss is None:
        raise RuntimeError("CUDA fused-loss runner did not select pinned Liger path")
    if model.lm_head.weight is not model.token_emb.weight:
        raise RuntimeError("tied lm-head identity drift")
    return {"liger_selected_on_cuda": True, "liger_kernel_version": importlib.metadata.version("liger-kernel"), "packaging_version": importlib.metadata.version("packaging"), "tied_lm_head_identity_preserved": True, "cuda_materializes_full_logits": False}
...
def h100_preflight(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    from architectures.cortex_s.production_scan_integration_v1 import build_memory_lean_grouped_triton_scan_cortex_100m, memory_lean_grouped_triton_scan_training_builder

    source, tree, harness = _validate_payload(source_sha, source_tree, harness_sha)
    volume.reload()
    root = Path(RESULT_ROOT)
    marker_path, result_path = root / "H100_DISPATCH_CONSUMED.json", root / "RESULT.json"
    if not marker_path.exists() or result_path.exists():
        raise RuntimeError("invalid v8 single-use H100 state")
    marker = json.loads(marker_path.read_text(encoding="utf-8"))
    for key, expected in {"status": "H100_DISPATCH_CONSUMED", "source_sha": source, "source_tree": tree, "harness_sha": harness, "engineering_seed": ENGINEERING_SEED, "result_root": RESULT_ROOT, "h100_allocation_started": False}.items():
        if marker.get(key) != expected:
            raise RuntimeError(f"dispatch marker mismatch for {key}")
    if not torch.cuda.is_available() or torch.cuda.get_device_capability(torch.device("cuda"))[0] < 9:
        raise RuntimeError("v8 systems preflight requires H100/SM90 CUDA")
    if not triton_available():
        raise RuntimeError("Triton is unavailable")
    _dependency_smoke()
    if _sha256(_training_module_and_path()[1]) != TRAIN_IMPLEMENTATION_SHA256:
        raise RuntimeError("production training implementation drift")
    if int(training_module.CALIBRATION_WARMUP_STEPS) != CALIBRATION_WARMUP_STEPS or int(training_module.CALIBRATION_STEPS) != MEASURED_STEPS:
        raise RuntimeError("calibration step contract drift")
    ...
    try:
        ...
        proof_model = build_memory_lean_grouped_triton_scan_cortex_100m().to(torch.device("cuda"))
        dispatcher_proof = _prove_triton_dispatch(proof_model, train_data, torch.device("cuda"))
        ...
        try:
            with memory_lean_grouped_triton_scan_training_builder():
                with fused_linear_ce_training_builder():
                    raw = training_module.run_h100_calibration(data_dir=DATA_DIR, output_path="/tmp/cortex_s_v8_fused_linear_ce_packaging_calibration.json", source_sha=source)
        finally:
            training_module.CALIBRATION_SEED = original_seed

        if raw.get("execution") != "compiled" or raw.get("calibration_seed") != ENGINEERING_SEED:
            raise RuntimeError("compiled/fresh-seed execution contract failed")
        if raw.get("measured_steps") != MEASURED_STEPS or raw.get("measured_tokens") != MEASURED_TOKENS:
            raise RuntimeError("measured calibration geometry drift")
        tps = float(raw["training_tokens_per_second"])
        compile_seconds = float(raw["compile_seconds"])
        projected = float(raw["projected_full_seconds"])
        expected_projection = TRAIN_TOKENS / tps * PROJECTION_OVERHEAD_MULTIPLIER + compile_seconds
        if not math.isfinite(tps) or tps <= 0 or not math.isclose(projected, expected_projection, rel_tol=1e-12, abs_tol=1e-9):
            raise RuntimeError("throughput/projection contract failed")
        expected_gate = projected <= MAX_PROJECTED_FULL_SECONDS
        if bool(raw["gates"].get("projected_full_seconds_le_budget")) != expected_gate:
            raise RuntimeError("8500-second gate reporting drift")
        status = "SYSTEMS_PREFLIGHT_PASS" if raw.get("status") == "PASS" and dispatcher_proof["legacy_fallback_calls"] == 0 and fused_cuda_proof["liger_selected_on_cuda"] else "SYSTEMS_PREFLIGHT_FAIL"
        result = {
            "status": status,
            "classification": "ENGINEERING_SYSTEMS_PREFLIGHT_V8_FUSED_LINEAR_CE_PACKAGING_ONLY",
            "phase": PHASE,
            "source_sha": source,
            "source_tree": tree,
            "harness_sha": harness,
            ...
            "calibration": {"execution": raw["execution"], "compile_mode": raw["compile_mode"], "compile_seconds": compile_seconds, "measured_steps": raw["measured_steps"], "measured_tokens": raw["measured_tokens"], "measured_seconds": raw["measured_seconds"], "training_tokens_per_second": tps, "projected_training_seconds": raw["projected_training_seconds"], "projection_overhead_multiplier": raw["projection_overhead_multiplier"], "projected_full_seconds": projected, "peak_vram_gb": raw["peak_vram_gb"], "last_train_loss": raw["last_train_loss"], "gates": raw["gates"]},
            "full_training_authorized": False,
            "scientific_claim_authorized": False,
        ...
        failure = {"status": "SYSTEMS_PREFLIGHT_ERROR", "classification": "ENGINEERING_SYSTEMS_PREFLIGHT_V8_FUSED_LINEAR_CE_PACKAGING_ERROR", "phase": PHASE, "source_sha": source, "source_tree": tree, "harness_sha": harness, "engineering_seed": ENGINEERING_SEED, "result_root": RESULT_ROOT, "error": f"{type(exc).__name__}: {exc}", "full_training_authorized": False, "scientific_claim_authorized": False, "seed_8100_used": False, "reserved_scientific_seeds_used": False}
