...
"""CHM-v1 #969 corrected optimized batched retrieval, CPU/systems only.

This additive module audits and fixes one integration discrepancy: the generic
batched transport calls ``state.index().indexed_search`` and therefore bypasses
``OptimizedEpisodicState.optimized_index()``.  The helpers here preserve the
existing batched host/device transport shape while explicitly executing the
#957/#958 ``ClippedBoundPrototype`` traversal.

Nothing in this module launches training, allocates GPU/Modal compute, consumes
scientific seeds, mutates a launcher/workflow, or changes the frozen scientific
...
def validate_diagnostic_seed(seed: int) -> int:
    """Refuse every scientific seed used by the CHM-v1 research sequences."""

    seed = int(seed)
    if seed in BLOCKED_SCIENTIFIC_SEEDS:
        raise RuntimeError(
            f"scientific seed refused by zero-GPU #969 systems diagnostic: {seed}"
        )
    return seed
...
def retrieve_many_optimized_exact(
    ...
    *,
    ...
    """Batched exact retrieval that explicitly exercises the clipped traversal.

    Search remains scalar and deterministic on CPU, matching the existing
    batched transport contract. Query transfer and selected-value transfer stay
    coalesced; only the indexed-search implementation changes from inherited
    ``ExactEpisodicIndex.indexed_search`` to
    ``ClippedBoundPrototype.indexed_search``.
    """

    if not isinstance(state, OptimizedEpisodicState):
        raise TypeError("state must be OptimizedEpisodicState")
    if queries.ndim != 2:
    ...
    if verify_indexed_exactness:
        ...
        flat_results = tuple(
            optimized.frozen.flat_search(query) for query in query_matrix
        )
...
def forward_session_chunk_optimized_batched_transport(
    ...
    *,
    ...
    """Two-hop chunk inference using the corrected optimized batched path."""
    ...
    if any(not isinstance(state, OptimizedEpisodicState) for state in states):
        raise TypeError("all states must be OptimizedEpisodicState")
    ...
    if update_memory:
        for batch_index, state in enumerate(states):
            ...
            stats.state_payload_bytes = max(
                stats.state_payload_bytes,
                state.payload_bytes(),
            )
    return logits, stats
