...
def fit_basis(joint,k=K):
    x=joint.reshape(-1,2,joint.shape[-2],joint.shape[-1])
    means=[]; comps=[]; explained=[]
    for ch in range(2):
        P=patch9(x,ch).reshape(-1,9).astype(np.float64)
        mu=P.mean(0); C=P-mu; cov=(C.T@C)/max(1,len(C)-1)
        vals,vecs=np.linalg.eigh(cov); order=np.argsort(vals)[::-1]; vals=vals[order]; vecs=vecs[:,order]
        W=vecs[:,:k].T.copy()
        for j in range(k):
            idx=int(np.argmax(np.abs(W[j])))
            if W[j,idx] < 0: W[j]*=-1
        means.append(mu.astype(np.float32)); comps.append(W.astype(np.float32)); explained.append(float(vals[:k].sum()/max(vals.sum(),1e-30)))
    return {'means':np.stack(means),'components':np.stack(comps),'explained_variance':explained}
...
REGISTERED=[
Seeds(818180001,818180002,818180003,818180004,818180005,818180006,818180011,818180012,818180013,818180021,818180022,818180023,818180024,818180031,818180032,818180033,818180034,818180041,818180042,818180043,818180044),
Seeds(818180101,818180102,818180103,818180104,818180105,818180106,818180111,818180112,818180113,818180121,818180122,818180123,818180124,818180131,818180132,818180133,818180134,818180141,818180142,818180143,818180144),
Seeds(818180201,818180202,818180203,818180204,818180205,818180206,818180211,818180212,818180213,818180221,818180222,818180223,818180224,818180231,818180232,818180233,818180234,818180241,818180242,818180243,818180244)]
...
def main():
    ...
    if a.smoke:
        s=Seeds(998180001,998180002,998180003,998180004,998180005,998180006,998180011,998180012,998180013,998180021,998180022,998180023,998180024,998180031,998180032,998180033,998180034,998180041,998180042,998180043,998180044);r=run_rep(s,grid=8,train_n=12,eval_n=5,code_sizes=(32,8),iters=3,max_points=3000);r['status']='SMOKE_ONLY_NONREGISTERED_SEED'
    else:
        if a.rep not in (1,2,3): raise SystemExit('--rep must be 1,2,3')
        r=run_rep(REGISTERED[a.rep-1]);r['replicate']=a.rep;r['status']='PHASE18_LEARNED_STENCIL_BASIS_REGISTERED_CONFIRMATION'
