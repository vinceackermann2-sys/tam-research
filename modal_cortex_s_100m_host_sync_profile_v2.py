...
PHASE = "host-sync-profile-v2"
TRIGGER_TITLE = "[modal-cortex-s-100m-host-sync-profile-v2]"
ENGINEERING_SEED = 2_026_091_006
RESULT_ROOT = "/vol/cortex-s-v0/100m-2b/host-sync-profile-v2"
...
APP_NAME = "cortex-s-v0-100m-host-sync-profile-v2"
...
def _validate_payload(source_sha: str, source_tree: str, harness_sha: str) -> tuple[str, str, str]:
    ...
    if ENGINEERING_SEED in forbidden:
        raise RuntimeError("host-sync v2 engineering seed is retired, consumed, or scientifically reserved")
...
def verify_zero_gpu(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if root.exists():
        raise RuntimeError("reserved host-sync attribution v2 namespace already exists; fail closed")
    ...
    payload = {
        "status": "PASS",
        "classification": "ZERO_GPU_HOST_SYNC_ATTRIBUTION_V2_GATE",
        "phase": PHASE,
...
def reserve_h100_dispatch(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    if not zero_path.exists():
        raise RuntimeError("zero-GPU attribution v2 gate is missing")
    ...
    for key, value in {"source_sha": source, "source_tree": tree, "harness_sha": harness, "status": "PASS"}.items():
        if zero.get(key) != value:
            raise RuntimeError(f"zero-GPU attribution v2 gate mismatch for {key}")
    if marker_path.exists() or result_path.exists():
        raise RuntimeError("host-sync attribution v2 engineering attempt is already consumed")

    marker = {
        "status": "H100_DISPATCH_CONSUMED",
        "classification": "ENGINEERING_SINGLE_ATTEMPT_MARKER_HOST_SYNC_ATTRIBUTION_V2",
        "phase": PHASE,
        "trigger_title": TRIGGER_TITLE,
        "source_sha": source,
        "source_tree": tree,
...
def h100_profile(source_sha: str, source_tree: str, harness_sha: str) -> str:
    ...
    from architectures.cortex_s.host_sync_profile_v2 import run_host_sync_profile
    from architectures.cortex_s.production_scan_integration_v1 import build_memory_lean_grouped_triton_scan_cortex_100m
    ...
    if result_path.exists():
        raise RuntimeError("host-sync attribution v2 result already exists; refusing rerun")
    ...
    payload = {
        ...
        "full_training_authorized": False,
        "scientific_claim_authorized": False,
        "note": "Engineering source attribution only; no production optimization, full 2B run, or scientific claim is authorized by this result.",
    }
