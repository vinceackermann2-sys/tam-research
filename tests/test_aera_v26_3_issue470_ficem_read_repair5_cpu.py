...
SOURCE_MAIN = "a358408c87d26ef447b1cd43ec74de2eae88d03c"
...
def _bf16_logit_branch(kernel: str) -> str:
    return kernel.split("        if IS_BF16:\n", 1)[1].split(
        "        else:\n            # Frozen pre-#426", 1
    )[0]


def test_issue470_protocol_freezes_repair5_scope_and_authorization():
...
def test_issue470_keeps_frozen_geometry_and_one_kernel_launch_topology():
    source = BACKEND_PATH.read_text()
...
def test_issue470_bf16_logit_path_matches_actual_autocast_visibility():
    kernel = _kernel_source()
    bf16 = _bf16_logit_branch(kernel)
...
def test_issue470_bf16_weights_have_one_visible_boundary_then_fp32_renormalization():
    kernel = _kernel_source()
    ...
    # The validity-masked values remain BF16-visible, but the sum, denominator and
    # final normalized weights must not be rounded back to BF16.
    assert "            ).to(tl.bfloat16)\n            denominator" not in section
    assert "denominator = tl.maximum(valid_weight_sum, 1.0e-9).to(tl.bfloat16)" not in section
    assert "denominator.to(tl.bfloat16)" not in section
    for i in range(4):
        assert f"weight{i} = (" not in section


def test_issue470_bf16_products_and_recalled_are_fp32_without_repair4_rounding():
    kernel = _kernel_source()
    section = kernel.split(
...
def test_issue470_preserves_float32_path_and_learned_out_projection_boundary():
    source = BACKEND_PATH.read_text()
    kernel = _kernel_source()
...
def test_issue470_has_no_hidden_fallback_state_growth_or_training_path():
    source = BACKEND_PATH.read_text()
    forbidden = (
        "torch.topk(",
        "torch.softmax(",
...
def test_issue470_preserves_frozen_probe_blob_and_no_probe_edit():
    assert _git_blob_sha(PROBE_PATH) == FROZEN_ISSUE418_PROBE_BLOB
    assert SOURCE_MAIN == "a358408c87d26ef447b1cd43ec74de2eae88d03c"
