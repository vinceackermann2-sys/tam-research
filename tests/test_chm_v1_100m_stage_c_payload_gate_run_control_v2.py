...
RUNNER = ROOT / "modal_chm_v1_100m_stage_c_payload_gate_1017_v2.py"
WORKFLOW = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-v2.yml"
...
def test_scientific_workflow_is_issue_only_one_shot_and_authority_bound() -> None:
    ...
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v2]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V2" in s
...
def test_authority_audit_is_cpu_inspect_only() -> None:
    ...
    assert "PAYLOAD_GATE_AUTHORITY_AUDIT_V2_PASS" in s
...
def test_v2_comment_recording_never_uses_plain_scalar_with_hash() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v2_has_fresh_namespace_and_never_reuses_v1_trigger() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    assert "/issue-1017/v2" in runner
    assert "/issue-1017/v1" not in runner
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v2]" in scientific
