...
def test_scientific_workflow_is_issue_only_one_shot_and_authority_bound() -> None:
    ...
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v4]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V4" in s
...
def test_authority_audit_is_cpu_inspect_only() -> None:
    s=AUDIT.read_text()
    assert "workflow_dispatch" not in s
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v4]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    ...
    assert "PAYLOAD_GATE_AUTHORITY_AUDIT_V4_PASS" in s
...
def test_v4_comment_recording_never_uses_plain_scalar_with_hash() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v4_has_fresh_namespace_and_never_reuses_v1_trigger() -> None:
...
def test_v4_workflows_have_real_block_scalars_and_retire_v1_v2_identities() -> None:
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v4_runner_binds_dual_account_identity_into_durable_evidence() -> None:
...
def test_v4_gpu_entry_requires_same_reservation_account_before_consumption() -> None:
    s = RUNNER.read_text(encoding="utf-8")
    run = _func(s, "run_diagnostic")
...
def test_v4_scientific_workflow_revalidates_only_authority_bound_account() -> None:
...
def test_v4_audit_selects_account_once_and_is_still_inspect_only() -> None:
...
def test_v4_workflows_bind_exact_dual_account_blobs() -> None:
    for path in (WORKFLOW, AUDIT):
