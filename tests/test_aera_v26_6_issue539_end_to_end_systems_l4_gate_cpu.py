...
LAUNCHER = ROOT / "modal_aera_v26_6_issue539_end_to_end_systems_app.py"
WORKFLOW = ROOT / ".github/workflows/aera-v26-6-issue539-e2e-systems-l4.yml"
...
def test_issue539_freezes_exact_evaluator_and_harness_blobs() -> None:
    assert _git_blob_sha(Path(evaluator.__file__)) == "9d5a3c31f4a3862f96b957540baa2e0ec6a84c6b"
    assert _git_blob_sha(LAUNCHER) == "4663d86e9dc2f8eedb68213cf673ce1f80e15574"
    assert _git_blob_sha(WORKFLOW) == "4e03f6e42c7635f4779f17aaa8d30488d968bc1f"


def test_issue539_preserves_frozen_systems_decision_surface() -> None:
...
def test_issue539_candidate_is_exact_primitive_passed_v26_6_backend() -> None:
    protocol = evaluator.issue530_systems_protocol()
...
def test_issue539_launcher_is_duplicate_safe_one_l4_result_before_marker() -> None:
    source = LAUNCHER.read_text()
    assert 'APP_NAME = "tam-research-aera-v26-6-issue539-end-to-end-systems"' in source
    assert 'VOLUME_NAME = "tam-research-data"' in source
    assert 'RESULT_PATH = "/vol/aera-v26/issue539-end-to-end-systems/result.json"' in source
    assert "MAX_GPU_SECONDS = 600" in source
    assert 'SOURCE_MAIN = "c4400a56ab293588f44d678965f577698fd345cf"' in source
    assert 'SOURCE_TREE = "d0ac3b9a82268d6707642f1d5e5b3a148bb309e3"' in source
    ...
    assert "refusing duplicate issue539" in source
    assert "modal.deploy" not in source
    assert "torch.optim" not in source
    assert ".backward(" not in source


def test_issue539_workflow_is_owner_attempt1_exact_bound_no_retry() -> None:
    source = WORKFLOW.read_text()
    assert "types: [opened]" in source
    assert "workflow_dispatch" not in source
    assert "cancel-in-progress: false" in source
    assert "github.event.issue.user.login == github.repository_owner" in source
    assert "[aera-v26-6-issue539-e2e-systems-l4]" in source
    ...
    assert "git merge-base --is-ancestor c4400a56ab293588f44d678965f577698fd345cf HEAD" in source
    assert 'git hash-object tam_research/aera_v26_6_issue530_end_to_end_systems.py' in source
