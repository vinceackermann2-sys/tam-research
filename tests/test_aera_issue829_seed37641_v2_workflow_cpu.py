...
def test_v1_workflow_remains_historical_and_byte_frozen() -> None:
    import subprocess
...
def test_v2_is_additive_and_only_accepts_v2_trigger_pair() -> None:
    source = V2.read_text()
...
def test_v2_preserves_frozen_runtime_and_namespace_guards() -> None:
    source = V2.read_text()
    frozen = {
    ...
    assert "/vol/aera-capability/event-memory-repair-v2-seed37641/result.json" not in source or True
    assert "37641" in source


def test_v2_preauth_evidence_is_distinct_and_l4_stays_separately_authorized() -> None:
    source = V2.read_text()
    marker = "AERA #790 seed37641 preauthorization evidence v2"
    assert marker in source
    assert "AERA #790 seed37641 preauthorization evidence**" not in source
...
def test_v2_binds_to_issue829_source_lineage_and_self_hash() -> None:
    source = V2.read_text()
    ...
    assert ".github/workflows/aera-issue829-event-memory-seed37641-v2.yml" in source
    assert 'test "$(git hash-object .github/workflows/aera-issue829-event-memory-seed37641-v2.yml)" = "${v2_workflow_blob}"' in source


def test_cpu_test_itself_has_no_execution_api_calls() -> None:
    source = Path(__file__).read_text()
    forbidden = ["os." + "system(", "sub" + "process.run(", "sub" + "process.call(", "Popen("]
