...
RUNNER = ROOT / "modal_chm_v1_100m_stage_c_payload_gate_1017_v5.py"
WORKFLOW = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-v5.yml"
AUDIT = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v5.yml"
...
def test_scientific_workflow_is_issue_only_one_shot_and_authority_bound() -> None:
    ...
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v5]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V5" in s
...
def test_authority_audit_is_cpu_inspect_only() -> None:
    s=AUDIT.read_text()
    assert "workflow_dispatch" not in s
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v5]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    ...
    assert "PAYLOAD_GATE_AUTHORITY_AUDIT_V5_PASS" in s
...
def test_v5_comment_recording_never_uses_plain_scalar_with_hash() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v5_has_fresh_namespace_and_never_reuses_v1_trigger() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    assert "/issue-1017/v5" in runner
    assert "/issue-1017/v1" not in runner
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v5]" in scientific
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v1]" not in scientific
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v5]" in audit


def test_v5_workflows_have_real_block_scalars_and_retire_v1_v2_identities() -> None:
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v5_runner_binds_dual_account_identity_into_durable_evidence() -> None:
    ...
    assert 'DUAL_ACCOUNT_CLI_BLOB = "7d9bd073bedcb8bfafcde23e6f03772dddb6d50e"' in s


def test_v5_gpu_entry_requires_same_reservation_account_before_consumption() -> None:
    s = RUNNER.read_text(encoding="utf-8")
    run = _func(s, "run_diagnostic")
...
def test_v5_scientific_workflow_revalidates_only_authority_bound_account() -> None:
...
def test_v5_audit_selects_account_once_and_is_still_inspect_only() -> None:
...
def test_v5_workflows_bind_exact_dual_account_blobs() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v5_selector_calls_require_repo_root_pythonpath() -> None:
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    assert 'PYTHONPATH="$PWD" python scripts/modal_select_account.py --required-volume tam-research-data' in audit
    assert 'PYTHONPATH="$PWD" python scripts/modal_select_account.py --required-volume tam-research-data --force-account "$BOUND_MODAL_ACCOUNT"' in scientific
    for source in (scientific, audit):
        for line in source.splitlines():
            stripped = line.strip()
            if "python scripts/modal_select_account.py" in stripped:
                assert stripped.startswith('PYTHONPATH="$PWD" python scripts/modal_select_account.py'), stripped


def test_v5_workflows_never_reference_retired_v4_runner() -> None:
    for path in (WORKFLOW, AUDIT):
        source = path.read_text()
        assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v3.py" not in source
        assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v5.py" in source
