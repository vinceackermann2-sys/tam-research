...
def test_v5_workflows_never_reference_retired_v3_runner() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v5_selector_invocations_make_repo_root_importable() -> None:
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
...
def test_v5_never_reuses_retired_v4_execution_identity() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    ...
    assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v5.py" in scientific
    assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v5.py" in audit


def test_v5_preserves_aligned_v4_probe_semantics() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
