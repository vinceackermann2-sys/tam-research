...
ISSUE488_HISTORICAL_TEST = ROOT / "tests" / "test_aera_v26_4_issue488_ficem_write_gate_cpu.py"

SOURCE_MAIN = "b36c12415a63e942f5ddb26612ea30d7166aa6b2"
...
def test_issue495_freezes_governing_predecessor_and_reference_blobs() -> None:
    assert SOURCE_MAIN == "b36c12415a63e942f5ddb26612ea30d7166aa6b2"
...
def test_issue495_numeric_threshold_and_fixed_geometry_are_unchanged() -> None:
...
def test_issue495_both_duplicate_comparisons_use_input_dtype_visible_threshold() -> None:
    ...
    assert "float32" not in source.lower()


def test_issue495_preserves_newest_wins_stable_compaction_and_materialization() -> None:
    source = MODULE.read_text()
...
def test_issue495_keeps_pytorch_projection_similarity_and_training_paths_unchanged() -> None:
...
def test_issue495_protocol_records_only_explicit_threshold_visibility_repair() -> None:
    ...
    assert protocol["write_threshold_repair1_issue"] == REPAIR_ISSUE
...
def test_issue495_historical_issue488_contract_is_explicit_marker_only() -> None:
    source = ISSUE488_HISTORICAL_TEST.read_text()
    assert 'protocol.get("write_threshold_input_dtype_visibility_repair1") is not True' in source
    assert 'protocol["write_threshold_input_dtype_visibility_repair1"] is True' in source
    assert 'protocol["write_pre_repair_backend_blob"] == BACKEND_BLOB' in source
    assert 'protocol["duplicate_similarity"] == 0.95' in source
    assert 'protocol["write_tail_triton_launches_target"] == 2' in source
    assert "xfail" not in source.lower()
    assert "pytest.skip" not in source


def test_issue495_has_no_fallback_science_or_gate_path() -> None:
