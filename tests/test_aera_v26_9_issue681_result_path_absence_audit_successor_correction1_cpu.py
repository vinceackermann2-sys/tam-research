...
WORKFLOW = Path(".github/workflows/aera-v26-9-issue681-result-path-absence-audit-successor-correction1.yml")
TEST_FILE = Path("tests/test_aera_v26_9_issue681_result_path_absence_audit_successor_correction1_cpu.py")
AUDIT_LAUNCHER = Path("modal_aera_v26_9_issue678_issue665_result_path_absence_audit.py")
ISSUE678_WORKFLOW = Path(".github/workflows/aera-v26-9-issue678-issue665-result-path-absence-audit.yml")
...
EXPECTED_WORKFLOW_BLOB = "dc6751bdfecac68c06139b65db576c3ebbbf7928"
EXPECTED_AUDIT_LAUNCHER_BLOB = "5d3f9df21bbee2d2b7dc1491bb8d1ce4dde5ec76"
EXPECTED_ISSUE678_WORKFLOW_BLOB = "d6e70a36cd9d8311e94443ceaa6ddd85bc965b86"
...
MANIFEST_HEADING = "## #681 terminal classification of consumed PR #683 and correction1 filename-isolation freeze"
...
def test_filename_isolation_preserves_legacy_issue678_scope() -> None:
    assert "issue678" not in WORKFLOW.name
    assert "issue678" not in TEST_FILE.name
    ...
    assert issue678_files == sorted(
        (
            str(ISSUE678_WORKFLOW),
            str(AUDIT_LAUNCHER),
            str(ISSUE678_CPU_TEST),
        )
    )
...
def test_frozen_existing_bytes_are_unchanged() -> None:
    assert _blob_sha(AUDIT_LAUNCHER) == EXPECTED_AUDIT_LAUNCHER_BLOB
    assert _blob_sha(ISSUE678_WORKFLOW) == EXPECTED_ISSUE678_WORKFLOW_BLOB
...
def test_correction1_workflow_blob_and_manifest_schema() -> None:
    ...
    assert "SOURCE_MAIN=20df86c8778e803181742b63357d82c7205ceafa" in text
    assert "SOURCE_TREE=3eeb0b9c584cb7d071a9f79417c499d32b2be401" in text
    assert "CONSUMED_HEAD=39e6086d6f7101b59ad101cd6111e1c0e0a7035e" in text
    assert "CONSUMED_RUN=34057122722" in text
    assert "CONSUMED_JOB=101550941759" in text
...
def test_fresh_trigger_authorization_and_consumed_boundaries() -> None:
    ...
    assert "pulls/683" in text
    assert "actions/runs/34057122722" in text
    assert "actions/jobs/101550941759" in text
    assert "39e6086d6f7101b59ad101cd6111e1c0e0a7035e" in text
...
def test_only_readonly_audit_launcher_can_execute() -> None:
    ...
    assert text.index("Verify issue681 correction1 read-only successor boundary") < text.index(command)
...
def test_reused_audit_launcher_remains_zero_gpu_preflight_only() -> None:
