...
RUNNER = ROOT / "modal_chm_v1_100m_stage_c_payload_gate_1017_v8.py"
WORKFLOW = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-v8.yml"
AUDIT = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v8.yml"
...
def test_runner_frozen_checkpoint_only_contract() -> None:
    ...
    for needle in (
        ...
        'RESULT_ROOT = "/vol/chm-v1/100m-stage-c-payload-gate/issue-1017/v8"',
...
def test_scientific_workflow_is_issue_only_one_shot_and_authority_bound() -> None:
    ...
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v8]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V8" in s
...
def test_authority_audit_is_cpu_inspect_only() -> None:
    s=AUDIT.read_text()
    assert "workflow_dispatch" not in s
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v8]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    ...
    assert "PAYLOAD_GATE_AUTHORITY_AUDIT_V8_PASS" in s
...
def test_v8_comment_recording_never_uses_plain_scalar_with_hash() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v8_has_fresh_namespace_and_never_reuses_v1_trigger() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    assert "/issue-1017/v8" in runner
    assert "/issue-1017/v1" not in runner
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v8]" in scientific
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v1]" not in scientific
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v8]" in audit


def test_v8_workflows_have_real_block_scalars_and_retire_v1_v2_identities() -> None:
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v8_runner_binds_dual_account_identity_into_durable_evidence() -> None:
...
def test_v8_gpu_entry_requires_same_reservation_account_before_consumption() -> None:
    s = RUNNER.read_text(encoding="utf-8")
    run = _func(s, "run_diagnostic")
...
def test_v8_scientific_workflow_revalidates_only_authority_bound_account() -> None:
    ...
    selector_name = "- name: Revalidate exact authority-bound Modal account"
    verify_name = "- name: Verify revalidated Modal account matches frozen authority"
    activate_name = "- name: Activate authority-bound Modal account"
    selector_start = s.index(selector_name)
    verify_start = s.index(verify_name)
    activate_start = s.index(activate_name)
    selector_block = s[selector_start:verify_start]
    verify_block = s[verify_start:activate_start]

    assert "id: account" in selector_block
    assert "steps.account.outputs" not in selector_block
    assert 'SELECTED_MODAL_ACCOUNT: ${{ steps.account.outputs.selected_account }}' in verify_block
    assert 'SELECTED_MODAL_WORKSPACE: ${{ steps.account.outputs.selected_workspace_name }}' in verify_block
    assert 'test "$SELECTED_MODAL_ACCOUNT" = "$BOUND_MODAL_ACCOUNT"' in verify_block
    assert 'test "$SELECTED_MODAL_WORKSPACE" = "$BOUND_MODAL_WORKSPACE"' in verify_block
...
def test_v8_audit_selects_account_once_and_is_still_inspect_only() -> None:
...
def test_v8_workflows_bind_exact_dual_account_blobs() -> None:
    for path in (WORKFLOW, AUDIT):
...
def test_v8_workflows_never_reference_retired_v3_runner() -> None:
    for path in (WORKFLOW, AUDIT):
        source = path.read_text()
        assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v3.py" not in source
        assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v8.py" in source


def test_v8_selector_invocations_make_repo_root_importable() -> None:
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
...
def test_v8_never_reuses_retired_v4_execution_identity() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
    ...
    assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v8.py" in scientific
    assert "modal_chm_v1_100m_stage_c_payload_gate_1017_v8.py" in audit


def test_v8_preserves_aligned_v4_probe_semantics() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v8_never_reuses_retired_v5_execution_identity() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    scientific = WORKFLOW.read_text(encoding="utf-8")
    audit = AUDIT.read_text(encoding="utf-8")
...
def test_v8_audit_rejects_only_fresh_v8_authority_not_historical_authority() -> None:
    source = AUDIT.read_text(encoding="utf-8")
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V8" in source
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V6" not in source
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V7" not in source


def test_v8_preserves_v7_account_selection_then_following_verification_order() -> None:
    source = WORKFLOW.read_text(encoding="utf-8")
    selector_name = "- name: Revalidate exact authority-bound Modal account"
    verify_name = "- name: Verify revalidated Modal account matches frozen authority"
    activate_name = "- name: Activate authority-bound Modal account"
    selector_start = source.index(selector_name)
    verify_start = source.index(verify_name)
    activate_start = source.index(activate_name)
    assert selector_start < verify_start < activate_start
    selector_block = source[selector_start:verify_start]
    verifier_block = source[verify_start:activate_start]
    assert "id: account" in selector_block
    assert "steps.account.outputs" not in selector_block
    assert "steps.account.outputs.selected_account" in verifier_block
    assert "steps.account.outputs.selected_workspace_name" in verifier_block
