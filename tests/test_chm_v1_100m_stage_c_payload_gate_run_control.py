...
RUNNER = ROOT / "modal_chm_v1_100m_stage_c_payload_gate_1017_v1.py"
WORKFLOW = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-v1.yml"
AUDIT = ROOT / ".github" / "workflows" / "modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v1.yml"
...
def test_scientific_workflow_is_issue_only_one_shot_and_authority_bound() -> None:
    ...
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-v1]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    assert "CHM_V1_100M_STAGE_C_PAYLOAD_GATE_FINAL_LAUNCHER_AUTHORITY_V1" in s
...
def test_authority_audit_is_cpu_inspect_only() -> None:
    s=AUDIT.read_text()
    assert "workflow_dispatch" not in s
    assert "[modal-chm-v1-100m-stage-c-payload-gate-1017-authority-audit-v1]" in s
    assert 'test "$RUN_ATTEMPT" = "1"' in s
    ...
    assert "PAYLOAD_GATE_AUTHORITY_AUDIT_V1_PASS" in s
