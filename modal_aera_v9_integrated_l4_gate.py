...
APP_NAME = "tam-research-aera-v9-integrated-l4"
...
def preflight() -> dict:
    ...
    model = HardwareAwareAERATextLMV9(small).eval().to(dtype=torch.bfloat16)
    if not all(isinstance(stage.experts, BMMHardSparseExpertBank) for stage in model.stages):
        raise RuntimeError("v9 preflight did not install BMM expert backend")
    ...
    logits = out["logits"]
    if logits.shape != (2, 31, small.vocab_size) or not torch.isfinite(logits.float()).all():
        raise RuntimeError("AERA-v9 stage-routing CPU preflight failed")
    ...
    return {"ok": True, "shape": list(logits.shape), "stage_execution": execution, "backend": "aera_v6_bmm"}
...
def run_gate() -> dict:
    ...
    torch.cuda.manual_seed_all(8900)
    torch.backends.cuda.matmul.allow_tf32 = True

    cfg = _cfg()
    base = HardwareAwareAERATextLMV9(cfg).to(device=device, dtype=dtype).eval()
    ...
    results = {}
    profiles = {"lean_skip": lean_skip, "lean_all": lean_all, "heavy": heavy, "dense": dense}
    ...
    result = {
        ...
        "implementation_revision": "aera_v9_dtype_safe_stage_merge",
        "forced_profiles": {
    ...
    print("AERA_V9_RESULT_JSON=" + json.dumps(result, separators=(",", ":")), flush=True)
...
def main():
    ...
    print("AERA_V9_PREFLIGHT_JSON=" + json.dumps(check, separators=(",", ":")), flush=True)
