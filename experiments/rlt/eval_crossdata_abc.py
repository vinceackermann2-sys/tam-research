...
PAIR_JOBS = (
    ("A", "rlt-compiled-paired-1m-modal-20260918-a"),
    ("B", "rlt-compiled-paired-1m-modal-20260919-b"),
    ("C", "rlt-compiled-paired-1m-modal-20260919-c"),
)
...
def evaluate_crossdata_abc(*, checkpoint_root: str, val_path: str) -> dict[str, Any]:
    ...
    t95 = 4.302652729911275  # df=2, two-sided 95%
    ci = [mean_delta - t95 * se, mean_delta + t95 * se]
    all_three = all(x < 0 for x in pair_deltas)

    if all_three:
        classification = "CROSSDATA_ABC_ALL_THREE_RLT_LOWER_NLL"
    elif mean_delta < 0:
        classification = "CROSSDATA_ABC_MEAN_RLT_LOWER_NLL_MIXED"
    else:
        classification = "CROSSDATA_ABC_NO_RLT_QUALITY_ADVANTAGE"
    ...
    return {
        ...
        "fresh_data": {
            ...
            "sampling": "deterministic non-overlapping 64-token next-token sequences from the fresh validation shard",
        },
        "pairs": results,
        "aggregate_across_model_seeds": {
            ...
            "t95_ci_pair_delta_nll_df2": ci,
        },
        "interpretation_ceiling": (
            "Cross-data evaluation of three independently trained model-seed pairs. "
            "All-three agreement would strengthen evidence that the 1M-token NLL effect generalizes, "
            "but it is still a small-model/single-dataset result and does not establish a general breakthrough."
        ),
    }
