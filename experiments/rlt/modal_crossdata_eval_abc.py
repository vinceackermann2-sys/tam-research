...
JOB_ID = "rlt-crossdata-eval-abc-modal-20260920-a"
...
app = modal.App("tam-rlt-crossdata-eval-abc-20260920-a")
...
def run_eval_remote(job_json: str) -> str:
    from experiments.rlt.eval_crossdata_abc import (
        BATCH_SIZE,
        DATA_SEED,
        EVAL_BATCHES,
        EVAL_TARGET_TOKENS,
    ...
    try:
        ...
        expected = {
            "schema": 1,
            "job_id": JOB_ID,
            "task": "crossdata_eval_abc",
            "requires_checkpoint_job_ids": [
                "rlt-compiled-paired-1m-modal-20260918-a",
                "rlt-compiled-paired-1m-modal-20260919-b",
                "rlt-compiled-paired-1m-modal-20260919-c",
            ],
        ...
        if job != expected:
            raise RuntimeError(f"cross-data job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("cross-data evaluation started without CUDA")
        ...
        data_dir = Path("/tmp/rlt-crossdata-eval-abc")
        ...
        evaluation = evaluate_crossdata_abc(
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote cross-data result must be base64 text")
    print("RLT_CROSSDATA_ABC_RESULT_B64=" + result)
