...
JOB_ID = "rlt-compiled-paired-4m-modal-20260923-c"
...
app = modal.App("tam-rlt-compiled-paired-4m-20260923-c")
...
def run_pair_remote(job_json: str) -> str:
    from experiments.rlt.train_compiled_pair_4m_c import SCIENTIFIC_SEED as TRAINER_SEED
    from experiments.rlt.train_compiled_pair_4m_c import train_compiled_pair_4m_c
    ...
    try:
        ...
        expected = {
            "schema": 1,
            "job_id": JOB_ID,
            "task": "compiled_paired_4m_replication_c",
            "scientific_seed": SCIENTIFIC_SEED,
            "requires_prior_pair_job_id": "rlt-compiled-paired-4m-modal-20260922-b",
            "requires_crossdata_job_id": "rlt-crossdata-eval-4m-ab-modal-20260923-a",
        ...
        if job != expected:
            raise RuntimeError(f"paired 4M C job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("Modal paired 4M C job started without CUDA")

        data_dir = Path("/tmp/rlt-compiled-pair-4m-data-c")
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote paired 4M C result must be base64 text")
    print("RLT_COMPILED_PAIR_4M_C_RESULT_B64=" + result)
