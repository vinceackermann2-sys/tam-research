...
JOB_ID = "rlt-compiled-paired-4m-modal-20260923-d"
...
app = modal.App("tam-rlt-compiled-paired-4m-20260923-d")
...
def run_pair_remote(job_json: str) -> str:
    from experiments.rlt.train_compiled_pair_4m_d import SCIENTIFIC_SEED as TRAINER_SEED
    from experiments.rlt.train_compiled_pair_4m_d import train_compiled_pair_4m_d
    ...
    try:
        ...
        expected = {
            "schema": 1,
            "job_id": JOB_ID,
            "task": "compiled_paired_4m_independent_train_shard_d",
        ...
        if job != expected:
            raise RuntimeError(f"paired 4M D job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("Modal paired 4M D job started without CUDA")

        data_dir = Path("/tmp/rlt-compiled-pair-4m-data-d")
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote paired 4M D result must be base64 text")
    print("RLT_COMPILED_PAIR_4M_D_RESULT_B64=" + result)
