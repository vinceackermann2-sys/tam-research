...
JOB_ID = "rlt-compiled-paired-4m-modal-20260922-b"
...
app = modal.App("tam-rlt-compiled-paired-4m-20260922-b")
...
def run_pair_remote(job_json: str) -> str:
    from experiments.rlt.train_compiled_pair_4m_b import SCIENTIFIC_SEED as TRAINER_SEED
    from experiments.rlt.train_compiled_pair_4m_b import train_compiled_pair_4m_b
    ...
    try:
        ...
        expected = {
            "schema": 1,
            "job_id": JOB_ID,
            "task": "compiled_paired_4m_replication_b",
            "scientific_seed": SCIENTIFIC_SEED,
            "requires_prior_pair_job_id": "rlt-compiled-paired-4m-modal-20260920-a",
            "requires_crossdata_job_id": "rlt-crossdata-eval-abc-modal-20260920-a",
        ...
        if job != expected:
            raise RuntimeError(f"paired 1M job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("Modal paired 1M job started without CUDA")

        data_dir = Path("/tmp/rlt-compiled-pair-1m-data")
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote paired 1M result must be base64 text")
    print("RLT_COMPILED_PAIR_4M_B_RESULT_B64=" + result)
