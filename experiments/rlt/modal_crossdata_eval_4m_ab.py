...
JOB_ID = "rlt-crossdata-eval-4m-ab-modal-20260923-a"
...
app = modal.App("tam-rlt-crossdata-eval-4m-ab-20260923-a")
...
def run_eval_remote(job_json: str) -> str:
    ...
    try:
        ...
        if job != expected:
            raise RuntimeError(f"cross-data 4M AB job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("cross-data 4M AB evaluation started without CUDA")
        ...
        data_dir = Path("/tmp/rlt-crossdata-eval-4m-ab")
        ...
        evaluation = evaluate_crossdata_4m_ab(
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote cross-data 4M AB result must be base64 text")
    print("RLT_CROSSDATA_4M_AB_RESULT_B64=" + result)
