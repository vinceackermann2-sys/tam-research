...
WARMUP_STEPS = 1
MEASURED_STEPS = 4
PRIOR_F_TPS = 5893.930399585457
PRIOR_F_GAP = 30.25874781771561
...
app = modal.App("tam-rlt-batchscale64-systems-20260916-g")
...
def run_profile_remote() -> str:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("batch64 systems profile requires CUDA")
    ...
    class ExactCompiledRLT(nn.Module):
        """D/F-path math; only diagnostic last_cache_lengths is omitted."""
    ...
    # Strict equivalence check at a small batch; benchmark path is unchanged D/F math.
    reference = new_rlt()
    ...
    x0 = torch.randint(0, rlt_cfg.vocab_size, (EQUIV_BATCH, SEQ_LEN), generator=eq_gen, dtype=torch.long).to(device)
    y0 = torch.randint(0, rlt_cfg.vocab_size, (EQUIV_BATCH, SEQ_LEN), generator=eq_gen, dtype=torch.long).to(device)
    ...
    if not semantic_equivalence:
        ...
        rlt_scale = rlt_result["tokens_per_second"] / PRIOR_F_TPS
        gap = transformer_result["tokens_per_second"] / rlt_result["tokens_per_second"]
        gap_improvement = PRIOR_F_GAP / gap
        classification = "PASS_BATCH64_SCALING_MATERIAL" if rlt_scale >= 1.25 else "PASS_BATCH64_SCALING_WEAK"
        rlt_result["speedup_vs_f_batch16"] = rlt_scale
    ...
    payload = {
        ...
        "profile": {
            ...
            "prior_f_batch16_rlt_tokens_per_second": PRIOR_F_TPS,
            "prior_f_transformer_gap_multiple": PRIOR_F_GAP,
        ...
        "derived": {
            "rlt_speedup_vs_f_batch16": rlt_scale,
            "transformer_gap_multiple": gap,
            "gap_improvement_vs_f": gap_improvement,
        },
        "rlt": rlt_result,
...
def main() -> None:
    ...
    print("RLT_BATCHSCALE64_RESULT_B64=" + result)
