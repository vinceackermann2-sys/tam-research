...
ENGINEERING_SEED = 20260930
SEQ_LEN = 64
BATCH_SIZE = 256
EQUIV_BATCH = 2
WARMUP_STEPS = 1
MEASURED_STEPS = 3
PRIOR_G_TPS = 35359.81801399832
PRIOR_G_GAP = 10.826896853394615
...
app = modal.App("tam-rlt-batchscale256-systems-20260916-h")
...
def run_profile_remote() -> str:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("batch256 systems profile requires CUDA")
    ...
    class ExactCompiledRLT(nn.Module):
        """Exact D/F/G execution math; only last_cache_lengths diagnostics are omitted."""
    ...
    # Strict semantic check at a small batch. The benchmark candidate is the same exact path.
    reference = new_rlt()
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        cand_loss = F.cross_entropy(
            cand_logits.float().reshape(-1, rlt_cfg.vocab_size), y0.reshape(-1)
        )
    ref_loss.backward()
    cand_loss.backward()
    ...
    for ref_p, cand_p in zip(reference.parameters(), candidate_base.parameters()):
        ...
        grad_max_abs = max(
            grad_max_abs,
            float((ref_p.grad - cand_p.grad).abs().max().item()),
        )
    ...
    if not semantic_equivalence:
        ...
        rlt_scale = rlt_result["tokens_per_second"] / PRIOR_G_TPS
        gap = transformer_result["tokens_per_second"] / rlt_result["tokens_per_second"]
        gap_improvement = PRIOR_G_GAP / gap
        classification = (
            "PASS_BATCH256_SCALING_MATERIAL"
            if rlt_scale >= 1.25
            else "PASS_BATCH256_SCALING_WEAK"
        )
        rlt_result["speedup_vs_g_batch64"] = rlt_scale
    ...
    payload = {
        ...
        "profile": {
            ...
            "prior_g_batch64_rlt_tokens_per_second": PRIOR_G_TPS,
            "prior_g_transformer_gap_multiple": PRIOR_G_GAP,
        ...
        "derived": {
            "rlt_speedup_vs_g_batch64": rlt_scale,
            "transformer_gap_multiple": gap,
            "gap_improvement_vs_g": gap_improvement,
        },
        "rlt": rlt_result,
...
def main() -> None:
    ...
    print("RLT_BATCHSCALE256_RESULT_B64=" + result)
