...
JOB_ID = "rlt-scale15m-paired-16m-modal-20260925-b"
SCIENTIFIC_SEED = 20_261_017
...
app = modal.App("tam-rlt-scale15m-paired-16m-20260925-b")
...
def run_pair_remote(job_json: str) -> str:
    from experiments.rlt.train_scale15m_pair_16m_b import SCIENTIFIC_SEED as TRAINER_SEED
    from experiments.rlt.train_scale15m_pair_16m_b import train_scale15m_pair_16m_b
    ...
    try:
        ...
        expected = {
            ...
            "requires_reference_pair_job_id": "rlt-compiled-paired-4m-modal-20260923-d",
                        "requires_prior_16m_job_id": "rlt-scale15m-paired-16m-modal-20260925-a",
        ...
        if job != expected:
            raise RuntimeError(f"scale15m paired 16M job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("Modal scale15m paired 16M job started without CUDA")

        data_dir = Path("/tmp/rlt-scale15m-paired-16m-data-b")
        ...
        pair = train_scale15m_pair_16m_b(
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote scale15m paired 16M result must be base64 text")
    print("RLT_SCALE15M_PAIR_16M_B_RESULT_B64=" + result)
