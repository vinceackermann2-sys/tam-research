...
JOB_ID = "rlt-compiled-paired-1m-modal-20260919-b"
SCIENTIFIC_SEED = 20_261_006
...
app = modal.App("tam-rlt-compiled-paired-1m-20260919-b")
...
def run_pair_remote(job_json: str) -> str:
    from experiments.rlt.train_compiled_pair_1m_b import SCIENTIFIC_SEED as TRAINER_SEED
    from experiments.rlt.train_compiled_pair_1m_b import train_compiled_pair_1m_b
    ...
    try:
        ...
        expected = {
            "schema": 1,
            "job_id": JOB_ID,
            "task": "compiled_paired_1m_replication_b",
        ...
        if job != expected:
            raise RuntimeError(f"paired 1M B job preregistration mismatch: {job}")
        ...
        if not runtime["cuda_available"]:
            raise RuntimeError("Modal paired 1M B job started without CUDA")

        data_dir = Path("/tmp/rlt-compiled-pair-1m-data-b")
...
def main(job_path: str) -> None:
    ...
    if not isinstance(result, str):
        raise TypeError("remote paired 1M B result must be base64 text")
    print("RLT_COMPILED_PAIR_1M_B_RESULT_B64=" + result)
