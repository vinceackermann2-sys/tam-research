...
app = modal.App("tam-rlt-compile-systems-20260915-a")
...
def run_profile_remote() -> str:
    ...
    from experiments.rlt.model import RecurrentLoopedTransformer, parameter_count
    from experiments.rlt.train_rlt import config_for_profile
    ...
    cfg = config_for_profile("tiny", SEQ_LEN)
    ...
    class CompilableRLT(nn.Module):
        ...
        def forward(self, tokens: torch.Tensor) -> torch.Tensor:
            ...
            memory_kv = [
                stage.cross_attn.precompute(memory) for stage in model.stages
            ]
    ...
    for _ in range(batch_count):
        x = torch.randint(
            0, cfg.vocab_size, (MICRO_BATCH, SEQ_LEN), generator=gen, dtype=torch.long
        ).to(device)
        y = torch.randint(
            0, cfg.vocab_size, (MICRO_BATCH, SEQ_LEN), generator=gen, dtype=torch.long
        ).to(device)
    ...
    # First prove that the side-effect-free candidate has identical forward math.
    reference = new_base()
    ...
    for step in range(MEASURED_STEPS):
        eager_last_loss = one_step(
            eager_model, eager_model, eager_opt, WARMUP_STEPS + step
        )
    torch.cuda.synchronize(device)
    ...
    # Compile the exact-math wrapper. Fullgraph is the preferred path; a graph-break
    # tolerant fallback is predeclared within this same one-shot engineering attempt.
    ...
    if compiled_callable is not None and compile_error is None:
        # One post-compilation warmup, excluded from measurement.
    ...
    compiled_tps = (
        None
        if compiled_seconds is None
        else measured_tokens / max(compiled_seconds, 1e-9)
    )
    ...
    payload = {
        ...
        "model": {
            "profile": "tiny",
            "parameters": parameter_count(new_base()),
            "seq_len": SEQ_LEN,
