...
ENGINEERING_SEED = 20261001
SEQ_LEN = 64
BATCH_SIZE = 512
EQUIV_BATCH = 2
WARMUP_STEPS = 1
MEASURED_STEPS = 2
PRIOR_H_TPS = 72087.0698111292
PRIOR_H_GAP = 5.208338214738094
...
app = modal.App("tam-rlt-batchscale512-systems-20260916-i")
...
def run_profile_remote() -> str:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("batch512 systems profile requires CUDA")
    device = torch.device("cuda")
    torch.set_float32_matmul_precision("high")

    rlt_cfg = RLTConfig(vocab_size=50_257, d_model=128, n_heads=4, n_stages=2,
                        max_seq_len=128, ff_mult=4, swa_window=32)
    transformer_cfg = ModelConfig(vocab_size=50_257, d_model=128, n_layers=3,
                                  n_heads=4, max_seq_len=128, ff_mult=4,
            ...
            for token_index in range(t):
                ...
                for layer_index, stage in enumerate(model.stages):
                    hidden, caches[layer_index] = stage.decode_step(
                        hidden, caches[layer_index], memory_kv[layer_index],
                        prefix_len, model.cfg.swa_window
                    )
                state = hidden[:, 0, :]
    ...
    for _ in range(WARMUP_STEPS + MEASURED_STEPS + 3):
        x = torch.randint(0, rlt_cfg.vocab_size, (BATCH_SIZE, SEQ_LEN),
                          generator=generator, dtype=torch.long).to(device)
        y = torch.randint(0, rlt_cfg.vocab_size, (BATCH_SIZE, SEQ_LEN),
                          generator=generator, dtype=torch.long).to(device)
        batches.append((x, y))
    ...
    eq_gen = torch.Generator(device="cpu").manual_seed(ENGINEERING_SEED + 99)
    x0 = torch.randint(0, rlt_cfg.vocab_size, (EQUIV_BATCH, SEQ_LEN), generator=eq_gen,
                       dtype=torch.long).to(device)
    y0 = torch.randint(0, rlt_cfg.vocab_size, (EQUIV_BATCH, SEQ_LEN), generator=eq_gen,
                       dtype=torch.long).to(device)
    ...
    def one_step(callable_model: Any, parameter_model: nn.Module, optimizer: Any,
                 offset: int, vocab_size: int) -> float:
    ...
    def compile_and_benchmark(kind: str) -> dict[str, Any]:
        if kind == "rlt":
            base = new_rlt(); callable_module: nn.Module = ExactCompiledRLT(base)
            vocab_size = rlt_cfg.vocab_size; params = parameter_count(base)
        else:
            base = new_transformer(); callable_module = base
            vocab_size = transformer_cfg.vocab_size; params = sum(p.numel() for p in base.parameters())
        ...
        torch.cuda.synchronize(device); compile_first_step = time.perf_counter() - t0
        ...
        result = {"parameters": params, "first_step_seconds_including_compile": compile_first_step,
    ...
    rlt_result = transformer_result = None
    try:
    ...
    gap = rlt_scale = gap_improvement = None
    if not semantic_equivalence:
        ...
        rlt_scale = rlt_result["tokens_per_second"] / PRIOR_H_TPS
        gap = transformer_result["tokens_per_second"] / rlt_result["tokens_per_second"]
        gap_improvement = PRIOR_H_GAP / gap
        classification = "PASS_BATCH512_SCALING_MATERIAL" if rlt_scale >= 1.25 else "PASS_BATCH512_SCALING_WEAK"
        rlt_result["speedup_vs_h_batch256"] = rlt_scale
    ...
    payload = {
        ...
        "engineering_seed": ENGINEERING_SEED, "started_unix": started,
        ...
        "profile": {"seq_len": SEQ_LEN, "batch_size": BATCH_SIZE,
                    "equivalence_batch_size": EQUIV_BATCH, "warmup_steps": WARMUP_STEPS,
                    "measured_steps": MEASURED_STEPS,
                    "prior_h_batch256_rlt_tokens_per_second": PRIOR_H_TPS,
                    "prior_h_transformer_gap_multiple": PRIOR_H_GAP,
                    "rlt_parameters": rlt_parameters},
        "semantic_equivalence": {"passed": semantic_equivalence,
                                 "logits_max_abs": logits_max_abs,
                                 "loss_abs": loss_abs, "grad_max_abs": grad_max_abs,
        ...
        "compile": {"fullgraph": True, "dynamic": False, "mode": "default",
                    "cuda_graphs_requested": False, "error": compile_error},
        "derived": {"rlt_speedup_vs_h_batch256": rlt_scale,
                    "transformer_gap_multiple": gap,
                    "gap_improvement_vs_h": gap_improvement},
        "rlt": rlt_result, "transformer": transformer_result,
    }
...
def main() -> None:
    ...
    print("RLT_BATCHSCALE512_RESULT_B64=" + result)
