...
app = modal.App("tam-rlt-compile-systems-20260915-d")
...
def run_profile_remote() -> str:
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        reference_loss = F.cross_entropy(reference_logits.float().reshape(-1, cfg.vocab_size), y0.reshape(-1))
        candidate_loss = F.cross_entropy(candidate_logits.float().reshape(-1, cfg.vocab_size), y0.reshape(-1))
    ...
    try:
        ...
        try:
            compiled_callable = torch.compile(wrapper, fullgraph=False, dynamic=False, mode="default")
            torch.cuda.synchronize(device)
            t0 = time.perf_counter()
            one_step(compiled_callable, compiled_base, compiled_opt, 0)
            torch.cuda.synchronize(device)
