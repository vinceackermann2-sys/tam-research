...
PAIRED_BATCH_SEED = 20_271_017
PAIRED_EVAL_SEED = 20_291_017
COMPILE_PROBE_SEED = 20_301_017
...
def train_scale15m_pair_16m_b(**kwargs: Any) -> dict[str, Any]:
    ...
    result["classification"] = (
        "PAIRED_SCALE15M_16M_RLT_LOWER_NLL_SINGLE_RUN_HINT"
        if delta < 0
        else "PAIRED_SCALE15M_16M_NO_RLT_QUALITY_ADVANTAGE"
    )
    ...
    result["interpretation_ceiling"] = (
        "Second independent fresh-seed exact-parameter-matched ~15.1M scale test at a 16,777,216-token budget. "
        "A positive result would show the replicated scale signal survives substantially longer optimization at roughly 2.16x "
        "parameter scale without parameter-count confounding. The run uses the same frozen FineWeb-Edu train/validation bytes as the prior 4M scale tests. It remains a single long-budget seed and "
        "cannot support a breakthrough claim until independently replicated and tested under "
