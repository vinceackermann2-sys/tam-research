...
APP_NAME = "tam-research-aera-v26-3-issue442-full-row-subgate-localize"
VOLUME_NAME = "tam-research-data"
RESULT_PATH = "/vol/aera-v26/issue442-repair3-full-row-subgate-localize/result.json"
MAX_GPU_SECONDS = 180
...
def _summary(result: dict) -> dict:
    decomposition = result["full_row_decomposition"]
    sequence = result["sequence_replay"]
    return {
        ...
        "projected_query_exact": decomposition["projected_query_exact"],
        "normalized_old_keys_exact": decomposition["normalized_old_keys_exact"],
        "pre_out_recalled_error": decomposition["pre_out_recalled_error"],
        "final_learned_out_error": decomposition["final_learned_out_error"],
        "fixture_unchanged_by_primary": result["fixture_unchanged_by_primary"],
        "sequence_target_fixture_matches_primary": result[
            "sequence_target_fixture_matches_primary"
        ],
        "sequence_target_matches_primary_correctness": result[
            "sequence_target_matches_primary_correctness"
        ],
        "sequence_target_pass": bool(sequence["target_correctness_row"]["pass"]),
        "sequence_target_false_subgates": sequence["target_false_subgates"],
        "preceding_rows": sequence["preceding_rows"],
...
def preflight() -> dict:
    ...
    from tam_research.aera_v26_3_repair3_full_row_subgate_localize import (
    ...
    if result_path.exists():
        raise RuntimeError(
            f"refusing duplicate #442 localization because result exists: {RESULT_PATH}"
        )
    ...
    if probe_blob != FROZEN_PROBE_GIT_BLOB:
        raise RuntimeError(f"#442 frozen probe blob drifted: {probe_blob}")
    if backend_blob != FROZEN_REPAIR3_BACKEND_GIT_BLOB:
        raise RuntimeError(f"#442 frozen repair3 backend blob drifted: {backend_blob}")
    ...
    print(
        "AERA_V26_ISSUE442_REPAIR3_FULL_ROW_SUBGATE_LOCALIZE_PREFLIGHT_JSON="
...
def run_localization_once() -> dict:
    ...
    print(
        "AERA_V26_ISSUE442_REPAIR3_FULL_ROW_SUBGATE_LOCALIZE_L4_START_JSON="
        + json.dumps(
            {
    ...
    volume.commit()
    print(
        "AERA_V26_ISSUE442_REPAIR3_FULL_ROW_SUBGATE_LOCALIZE_RESULT_JSON="
        + json.dumps(result, separators=(",", ":")),
        flush=True,
    )
    print(
        "AERA_V26_ISSUE442_REPAIR3_FULL_ROW_SUBGATE_LOCALIZE_SUMMARY_JSON="
...
def main() -> None:
    ...
    print(
        "AERA_V26_ISSUE442_REPAIR3_FULL_ROW_SUBGATE_LOCALIZE_PREFLIGHT_LOCAL="
