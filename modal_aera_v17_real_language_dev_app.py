...
APP_NAME = "tam-research-aera-v17-real-language-dev-seed8331"
VOLUME_NAME = "tam-research-data"
SEED = 8331
DATA_DIR = "/vol/data/tam100m-2b-curated-v1"
RUN_DIR = f"/vol/aera-real-language/v17-dev-seed{SEED}"
...
def preflight() -> dict:
    from tam_research.aera_real_language_v17_gpu import validate_protocol

    volume.reload()
    result = validate_protocol(DATA_DIR)
    print("AERA_V17_DEV_PREFLIGHT_JSON=" + json.dumps(result, separators=(",", ":")), flush=True)
    return result
...
def run_gate() -> dict:
    ...
    from tam_research.aera_real_language_v17_gpu import train_matched_pair
    ...
    volume.commit()
    print("AERA_V17_DEV_RESULT_JSON=" + json.dumps(result, separators=(",", ":")), flush=True)
    return result
...
def main() -> None:
    ...
    print("AERA_V17_DEV_PREFLIGHT_LOCAL=" + json.dumps(check, separators=(",", ":")), flush=True)
