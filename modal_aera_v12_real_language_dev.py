...
def preflight() -> dict:
    from tam_research.aera_real_language_v12_gpu import validate_protocol

    volume.reload()
    result = validate_protocol(DATA_DIR)
    print(
        "AERA_V12_DEV_PREFLIGHT_JSON=" + json.dumps(result, separators=(",", ":")),
        flush=True,
    )
    return result
...
def run_gate() -> dict:
    ...
    from tam_research.aera_real_language_v12_gpu import train_matched_pair
    ...
    volume.commit()
    print(
        "AERA_V12_DEV_RESULT_JSON=" + json.dumps(result, separators=(",", ":")),
        flush=True,
    )
    return result
...
def main() -> None:
    ...
    print(
        "AERA_V12_DEV_PREFLIGHT_LOCAL=" + json.dumps(check, separators=(",", ":")),
        flush=True,
    )
